...
SEQUENCE_LENGTH = 2            # Sliding-window history length for error features
...
EPOCHS = 2                     # Training epochs per dataset
LEARNING_RATE = 1e-3
...
def save_checkpoint(dataset_label: str, feature_dim: int, axis_models, scaler_X, axis_scalers, dataset_histories):
...
def train_on_dataset(dataset_label, mat_path, axis_models, scaler_X, axis_scalers, device):
...
def evaluate_on_test(mat_path, axis_models, scaler_X, axis_scalers, device):
...
def main():
    """Main training / evaluation routine for the sequential multi-dataset experiment."""
    ...
    for dataset_label, mat_file in TRAIN_DATASETS:
        ...
        save_checkpoint(dataset_label, feature_dim, axis_models, scaler_X, axis_scalers, histories)

    test_label, test_file = TEST_DATASET
    test_path = os.path.join(HERE, test_file)
    mse_controls = evaluate_on_test(test_path, axis_models, scaler_X, axis_scalers, device)
    print(
    ...
    print(f"\nEnd-to-end script runtime: {script_elapsed:.2f}s")
