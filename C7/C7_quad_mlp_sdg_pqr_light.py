# C7 (light): Load C5 model, run full-sequence sim, MSE + plots
...
def load_dataset(mat_path: str):
    ...
    X = np.hstack([ctrl[:-1, :], att_deg[:-1, :], pqr_deg_s[:-1, :]])
    Y = np.hstack([att_deg[1:, :],  pqr_deg_s[1:, :]])
    return X, Y, t[1:]
...
def main():
    ...
    fig1 = plt.figure(num="C7_light: Full Angles (True vs Pred)", figsize=(12, 6))
    for i, lbl in enumerate(['phi [deg]', 'theta [deg]', 'psi [deg]']):
        plt.subplot(3,1,i+1)
        plt.plot(t, A_true[:, i], label='True', lw=1)
        plt.plot(t, A_pred[:, i], '--', label='Pred', lw=1)
    ...
    # Full-sequence rates
    fig2 = plt.figure(num="C7_light: Full Rates (True vs Pred)", figsize=(12, 6))
    for i, lbl in enumerate(['p [deg/s]', 'q [deg/s]', 'r [deg/s]']):
        plt.subplot(3,1,i+1)
        plt.plot(t, R_true[:, i], label='True', lw=1)
        plt.plot(t, R_pred[:, i], '--', label='Pred', lw=1)
        plt.ylabel(lbl); plt.grid(alpha=0.3)
    plt.legend(loc='upper right'); plt.xlabel('Time [s]'); plt.tight_layout()
