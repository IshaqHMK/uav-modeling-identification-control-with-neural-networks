"""
C22 v1: Cyclic sequential PID RNN training across multiple datasets.

This variant runs short, repeated passes over D1 -> D2 -> D3 to alleviate
catastrophic forgetting. For each sequence length in SEQUENCE_LENGTHS:
    * Run CYCLE_COUNT cycles.
    * During each cycle, train 1 epoch on D1, then continue training the same
      models for 1 epoch on D2, then 1 epoch on D3.
    * After D3, evaluate on the held-out test dataset and log/save checkpoints.

Checkpoints are tagged with the dataset label, cycle index, and sequence length
so experiments can be compared easily.
"""
...
SEQUENCE_LENGTHS = [60]          # Provide one or more sequence lengths to iterate over
CYCLE_COUNT = 10                # Number of outer cycles (D1 -> D2 -> D3)
...
EPOCHS = 1                      # Training epochs per dataset per visit
LEARNING_RATE = 1e-3
...
def save_checkpoint(dataset_label: str, feature_dim: int, axis_models, scaler_X, axis_scalers,
                    dataset_histories, sequence_length: int, cycle_index: int):
    ...
    ckpt_path = os.path.join(
        "models",
        f"{SAVE_PREFIX}{dataset_label}_cycle{cycle_index}_SL_{sequence_length}_pid_rnn_model.pt"
    )
...
def run_sequence_length(sequence_length: int, device):
    """Execute the cyclic training pipeline for a single sequence length."""
    ...
    for cycle_idx in range(1, CYCLE_COUNT + 1):
        print(f"\n--- Cycle {cycle_idx}/{CYCLE_COUNT} (sequence length {sequence_length}) ---")
        cycle_start = time.perf_counter()

        for ds_order, (dataset_label, mat_file) in enumerate(TRAIN_DATASETS, start=1):
            ...
            # After processing the final dataset in the sequence, run a test evaluation.
            if ds_order == num_datasets:
                mse_controls = evaluate_on_test(test_path, axis_models, scaler_X, axis_scalers, device, sequence_length)
                print(
                    f"  Test dataset ({test_label}) MSE [(rad/s)^2]: "
                    f"u2={mse_controls[0]:.4e} | u3={mse_controls[1]:.4e} | u4={mse_controls[2]:.4e}"
                )

        cycle_elapsed = time.perf_counter() - cycle_start
        print(f"--- Cycle {cycle_idx} complete in {cycle_elapsed:.2f}s ---")

    seq_elapsed = time.perf_counter() - seq_start
    print(f"Sequence length {sequence_length} finished after {CYCLE_COUNT} cycles in {seq_elapsed:.2f}s")
