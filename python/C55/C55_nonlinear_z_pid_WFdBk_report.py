...
# Figure saving
SAVE_PREFIX = "C55_WFdBk_report_"
FIG_DIR = os.path.dirname(os.path.abspath(__file__))
REPORT_DIR = os.path.join(FIG_DIR, "c55_report")
FIG_SIZE = (8, 3)
# Only save report figures for these dataset labels.
REPORT_DATASET_LABELS = {"S21_W5", "S22_W5"}
...
def run_nonlinear_z_pid(wind_force=0.0, wind_start_time=20.0, aprbs_seed=None):
    ...
    cos_term_hist = np.zeros(NUM_SAMPLES)
    u1_noise_hist = np.zeros(NUM_SAMPLES)
...
def plot_dataset(dataset, dataset_label):
    """Step 1 plots: one plot per figure."""
    time = dataset["time"]

    fig = plt.figure(figsize=FIG_SIZE)
    plt.plot(time, dataset["z"], label="z", linewidth=1, color="b")
    plt.plot(time, dataset["z_ref"], "--", label="z_ref", linewidth=1, color="r")
    plt.title(f"{dataset_label}: Z Tracking", fontsize=11, fontweight="bold")
    ...
    plt.title(f"{dataset_label}: Control Input U1", fontsize=11)
    plt.xlabel("Time (s)", fontsize=10)
    plt.ylabel("U1 (N)", fontsize=10)
    plt.grid(True, linestyle="--", alpha=0.7)
    plt.legend(fontsize=8)
    plt.tight_layout()
    ...
    fig = plt.figure(figsize=FIG_SIZE)
    plt.plot(time, dataset["phi"], label="phi", linewidth=1, color="m")
    plt.plot(time, dataset["phi_ref"], "--", label="phi_ref", linewidth=1, color="k")
    plt.title(f"{dataset_label}: Roll", fontsize=11)
    plt.xlabel("Time (s)", fontsize=10)
    ...
    fig = plt.figure(figsize=FIG_SIZE)
    plt.plot(time, dataset["theta"], label="theta", linewidth=1, color="c")
    plt.plot(time, dataset["theta_ref"], "--", label="theta_ref", linewidth=1, color="k")
    plt.title(f"{dataset_label}: Pitch", fontsize=11)
    plt.xlabel("Time (s)", fontsize=10)
    ...
    fig = plt.figure(figsize=FIG_SIZE)
    plt.plot(time, dataset["psi"], label="psi", linewidth=1, color="tab:orange")
    plt.plot(time, dataset["psi_ref"], "--", label="psi_ref", linewidth=1, color="k")
    plt.title(f"{dataset_label}: Yaw", fontsize=11)
    plt.xlabel("Time (s)", fontsize=10)
    plt.ylabel("psi (rad)", fontsize=10)
    plt.grid(True, linestyle="--", alpha=0.7)
    plt.legend(fontsize=8)
    plt.tight_layout()
    ...
    # cos_term and u1_noise plots are intentionally omitted for the report.
...
def plot_learning_curve(history):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(REPORT_DIR, f"{SAVE_PREFIX}step2_learning_curve.png"), dpi=300)
    plt.close(fig)
...
def plot_pid_vs_model(dataset_label, time, z_ref, pid_data, model_data):
    """Step 3 plots: one plot per figure."""
    fig = plt.figure(figsize=FIG_SIZE)
    plt.plot(time, pid_data["z"], label="PID z", linewidth=1, color="b")
    plt.plot(time, model_data["z"], label="Model z", linewidth=1, color="g")
    plt.plot(time, z_ref, "--", label="z_ref", linewidth=1, color="r")
    plt.title(f"{dataset_label}: Z Tracking", fontsize=11, fontweight="bold")
    plt.xlabel("Time (s)", fontsize=10)
    plt.ylabel("Altitude (m)", fontsize=10)
    plt.grid(True, linestyle="--", alpha=0.7)
    plt.legend(fontsize=8)
    plt.tight_layout()
    fig.savefig(os.path.join(REPORT_DIR, f"{SAVE_PREFIX}{dataset_label}_step3_z_tracking.png"), dpi=300)
    plt.close(fig)

    fig = plt.figure(figsize=FIG_SIZE)
    plt.plot(time, pid_data["u1"], label="PID U1", linewidth=1, color="b")
    plt.plot(time, model_data["u1"], label="Model U1", linewidth=1, color="g")
    plt.title(f"{dataset_label}: Control Input U1", fontsize=11)
    plt.xlabel("Time (s)", fontsize=10)
    plt.ylabel("U1 (N)", fontsize=10)
    plt.grid(True, linestyle="--", alpha=0.7)
    plt.legend(fontsize=8)
    plt.tight_layout()
    fig.savefig(os.path.join(REPORT_DIR, f"{SAVE_PREFIX}{dataset_label}_step3_u1.png"), dpi=300)
    plt.close(fig)
...
def main():
    ...
    # No interactive display for report mode.
    ...
    for label in plot_labels:
        if label not in REPORT_DATASET_LABELS:
            continue
