...
Step 1:
  ...
  - Apply an APRBS reference scaled by three amplitude levels (<= 2.0).
...
# Dataset grid: 3 reference amplitudes (<= 2.0) x 3 wind magnitudes
REFERENCE_AMPLITUDES = [0.5, 1.0, 2.0]
WIND_LEVELS = [0.0, 1.0, 5.0]  # N


# Step 2 plotting config
PLOT_DATASET_LABEL = "ALL"  # "ALL" or a label like A1_W0 or A0p5_W1

# Figure saving
SAVE_PREFIX = "C51_WFdBk_"
FIG_DIR = os.path.dirname(os.path.abspath(__file__))
MODEL_SAVE_PREFIX = "C51_linear_z_pid_WFdBk_trainedGRUmodel"
...
# Base APRBS levels in [0, 1]; scaled later by REFERENCE_AMPLITUDES.
APRBS_AMP_LEVELS = np.array([0.1, 0.25, 0.5, 0.75, 1.0], dtype=float)
APRBS_ENV_DWELL_STEPS = 5000
APRBS_RAMP_STEPS = 0
...
def aprbs_generate_reference_array():
    num_samples = int(APRBS_TOTAL_TIME / APRBS_Ts)
    ...
    rng = np.random.default_rng(APRBS_RNG_SEED)
    env = np.zeros(num_samples, dtype=float)

    start_zero_steps = int(max(0.0, APRBS_START_ZERO_TIME) / APRBS_Ts)
    if start_zero_steps > 0:
        env[:start_zero_steps] = 0.0
        idx = start_zero_steps
    else:
        idx = 0
    ...
    while idx < num_samples:
        a_next = float(rng.choice(APRBS_AMP_LEVELS))

        if APRBS_RAMP_STEPS <= 0:
            ramp_len = 0
        else:
            ramp_len = int(min(APRBS_RAMP_STEPS, num_samples - idx))

        if ramp_len > 0:
            env[idx:idx + ramp_len] = np.linspace(a_prev, a_next, ramp_len, endpoint=False)
            idx += ramp_len

        dwell_len = int(min(APRBS_ENV_DWELL_STEPS, num_samples - idx))
        if dwell_len > 0:
...
def build_dataset_label(amplitude, wind_force):
    """Stable dataset label for plotting and saving figures."""
    return f"A{format_value(amplitude)}_W{format_value(wind_force)}"
...
def run_linear_z_pid(amplitude=1.0, wind_force=0.0, wind_start_time=20.0):
    ...
    for i in range(NUM_SAMPLES):
        # APRBS base is in [0, 1]; apply dataset amplitude scaling here.
        z_ref = amplitude * aprbs_ref[i]
...
def main():
    ...
    datasets = []
    for amplitude in REFERENCE_AMPLITUDES:
        for wind_force in WIND_FORCES:
            label = build_dataset_label(amplitude, wind_force)
            ds = run_linear_z_pid(
                amplitude=amplitude,
            ...
            print(f"{label}: amplitude={amplitude}, wind={wind_force} N, RMS error = {rms:.4f} m")
    ...
    ckpt = {
        ...
        "reference": {
            "type": "aprbs",
            "base_levels": APRBS_AMP_LEVELS.tolist(),
            "signed": APRBS_SIGNED,
            "hold_steps": APRBS_HOLD_STEPS,
    ...
    if PLOT_DATASET_LABEL == "ALL":
        ...
        if PLOT_DATASET_LABEL not in dataset_map:
            raise ValueError(f"Unknown PLOT_DATASET_LABEL '{PLOT_DATASET_LABEL}'. Use 'ALL' or a dataset label like A1_W0.")
