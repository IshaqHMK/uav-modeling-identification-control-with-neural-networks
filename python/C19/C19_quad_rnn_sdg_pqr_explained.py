"""
C19: RNN sequence model with extensive inline commentary to explain each operation.
This file mirrors C17 functionality but annotates the full workflow for study purposes.
"""
...
SAVE_PREFIX = "C19_"            # Prefix applied to every artifact saved to disk
...
def load_and_build_dataset(mat_path: str):
    ...
    ctrl = ctrl_full[:, 1:4]               # Retain only U2-U4 because U1 is not needed here

    att_rad = data['attitude_data']        # Orientation (phi, theta, psi) already in radians
    if 'gyro_data' not in data:            # Verify gyro data exists to avoid downstream crashes
...
def create_loaders(X_train, Y_train, X_val, Y_val):
    ...
    val_loader = DataLoader(val_ds, batch_size=BATCH_SIZE, shuffle=False, drop_last=False)      # Keep validation order stable
...
def train_model(model, train_loader, val_loader, device):
    ...
    for epoch in range(1, EPOCHS + 1):                                       # Iterate over epochs
        ...
        #if epoch % 20 == 0 or epoch == 1:                                    # Periodic logging of progress
