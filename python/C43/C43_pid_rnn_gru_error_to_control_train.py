...
TEST_DATASET = ("TEST", "C43_sim_dataset_test.mat")

SEQUENCE_LENGTH = 10           # History window length (tunable)
T_CROP_SECONDS = 50.0         # Time crop applied uniformly across datasets
TRAIN_FRACTION = 0.7
...
AXES = [
    ("altitude", "u1", "u1_thrust", "u1 [N]", 0),
    ("roll", "u2", "u2_rad_s", "u2 [deg/s]", 1),
    ("pitch", "u3", "u3_rad_s", "u3 [deg/s]", 2),
    ("yaw", "u4", "u4_rad_s", "u4 [deg/s]", 3),
]
...
def load_and_build_dataset(mat_path: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    att_rad = data['attitude_data']  # phi, theta, psi

    if 'reference_data' not in data:
        raise KeyError("MAT does not contain 'reference_data' (expected [alt, roll, pitch, yaw])")
    ref_all = data['reference_data']  # z, phi, theta, psi

    # Altitude measurement
    if 'altitudes' in data:
        alt_meas = np.asarray(data['altitudes']).squeeze()
        alt_meas = alt_meas[:, 0] if alt_meas.ndim > 1 else alt_meas
    elif 'states_history' in data:
        alt_meas = np.asarray(data['states_history'])[:, 4]
    else:
        raise KeyError("MAT does not contain 'altitudes' or 'states_history' for altitude measurement")

    # Build 4-axis measured vs reference arrays
    meas_full = np.column_stack([alt_meas, att_rad])
    ref_full = ref_all[:, :4]

    time_vec = data['sim_times'].ravel()

    lengths = [len(ctrl), len(meas_full), len(ref_full), len(time_vec)]
    min_len = min(lengths)
    if len(set(lengths)) != 1:
        ctrl = ctrl[:min_len]
        meas_full = meas_full[:min_len]
    ...
    error_rate[1:] = np.diff(error, axis=0) / dt_samples[:, None]

    error_integral = cumulative_trapezoid(error, time_vec, axis=0, initial=0.0)

    feature_stack = np.concatenate([error, error_rate, error_integral], axis=1)
    return feature_stack, ctrl, time_vec, error
...
def main():
    ...
    print("\nPer-dataset MSE [(mixed units)]:")
    for dataset_label in TRAIN_DATASETS:
        label = dataset_label[0]
        res = eval_results[label]
        print(
            f"  {label} | "
            f"train: {'/'.join(f'{v:.4e}' for v in res['train_mse'])} | "
            f"val: {'/'.join(f'{v:.4e}' for v in res['validation_mse'])} | "
            f"test: {'/'.join(f'{v:.4e}' for v in res['test_mse'])}"
        )

    print(
        f"\nHeld-out dataset ({TEST_DATASET[0]}) test MSE [(mixed units)]: "
        + " | ".join(f"{axis_name}={test_mse[idx]:.4e}" for axis_name, _, _, _, idx in AXES)
    )
    ...
    ckpt = {
        ...
        "feature_names": [
            "z_error_m", "phi_error_rad", "theta_error_rad", "psi_error_rad",
            "z_error_rate_m_s", "phi_error_rate_rad_s", "theta_error_rate_rad_s", "psi_error_rate_rad_s",
            "z_error_int_m_s", "phi_error_int_rad_s", "theta_error_int_rad_s", "psi_error_int_rad_s",
        ],
        "target_names": [target_name for _, _, target_name, _, _ in AXES],
        "units": {"error": "mixed (m/rad)", "control": "N and rad/s"},
        "train_fractions": {
