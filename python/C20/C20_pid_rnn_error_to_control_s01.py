"""
C20 s01: Independent-axis PID imitation with stacked LSTM regressors.

This script learns three separate sequence-to-command models for roll, pitch, and yaw PID outputs (u2, u3, u4).
It keeps the data preparation, scaling, plotting, and checkpoint structure from C20_pid_rnn_error_to_control_v3_clnd,
but replaces the shared multi-output RNN head with per-axis regressors so each channel can specialize.

Workflow:
1. Load the quadrotor flight log, crop to T_CROP_SECONDS, and build sliding-window sequences of attitude error -> PID commands.
2. Fit feature/target standardization and train an axis-specific RNN on each PID command channel.
...
SEQUENCE_LENGTH = 30            # Number of time steps used in each sliding window sequence
