...
MICRO_BATCH_SIZE = 512          # Chunk size used internally to emulate full-batch updates safely
# -------------------------------------------------------------------------------------
...
def train_model(model, X_train_tensor, Y_train_tensor, X_val_tensor, Y_val_tensor):
    """Train the RNN using gradient accumulation to emulate single full-batch updates safely."""
    ...
    total_train = X_train_tensor.size(0)
    total_val = X_val_tensor.size(0)
    micro_batch = min(MICRO_BATCH_SIZE, total_train)                         # Limit per-step size to avoid backend crashes

    for epoch in range(1, EPOCHS + 1):                                       # Iterate over epochs
        model.train()                                                        # Enable training mode (activates dropout, etc.)
        optimizer.zero_grad()                                                # Reset accumulated gradients

        train_loss_sum = 0.0                                                 # Accumulate loss for reporting
        for start in range(0, total_train, micro_batch):
            end = min(start + micro_batch, total_train)
            xb = X_train_tensor[start:end]
            yb = Y_train_tensor[start:end]
            preds = model(xb)
            loss = criterion(preds, yb)
            batch_sz = end - start
            (loss * (batch_sz / total_train)).backward()                     # Scale gradients to match full-batch average
        ...
        train_loss_value = train_loss_sum / total_train

        model.eval()                                                         # Switch to eval mode for validation pass
        val_loss_sum = 0.0
        with torch.no_grad():                                                # Disable gradient tracking during validation
            for start in range(0, total_val, micro_batch):
                end = min(start + micro_batch, total_val)
                xb = X_val_tensor[start:end]
                yb = Y_val_tensor[start:end]
                preds = model(xb)
                loss = criterion(preds, yb)
                val_loss_sum += loss.item() * (end - start)
        val_loss_value = val_loss_sum / total_val
