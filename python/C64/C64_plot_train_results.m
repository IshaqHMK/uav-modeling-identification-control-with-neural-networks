% C64 training MAT plotter (for SLURM headless runs)
% Loads C64_train_results.mat and regenerates C64 figures in MATLAB.

clear; clc;

base_dir = fileparts(mfilename('fullpath'));
mat_path = fullfile(base_dir, 'mat_results', 'C64_train_results.mat');
out_dir = fullfile(base_dir, 'c64_train_figures_matlab');
...
if isfield(S, 'step2_train_loss') && isfield(S, 'step2_validation_loss')
    ...
    xlabel('Epoch'); ylabel('MSE'); title('C64 Step 2 Roll Learning Curve');
    grid on; legend({'Train', 'Validation'}, 'Location', 'best');
    saveas(f, fullfile(out_dir, 'C64_step2_roll_learning_curve.png'));
    close(f);
end
...
for i = 1:numel(labels)
    ...
    if isfield(S, f_time)
        ...
        nexttile;
        plot(t, S.(sprintf('step1_%s_tau_x', tag)), 'LineWidth', 1);
        ...
        title('Pitch/Yaw'); xlabel('Time (s)'); ylabel('rad'); grid on;
        legend('\theta','\theta_{ref}','\psi','\psi_{ref}', 'Location', 'best');

        nexttile;
        plot(t, S.(sprintf('step1_%s_wind', tag)), 'r', 'LineWidth', 1);
        title('Wind disturbance'); xlabel('Time (s)'); ylabel('N'); grid on;

        saveas(f, fullfile(out_dir, sprintf('C64_%s_step1_pid_dataset.png', lbl)));
        close(f);
    end
    ...
    if isfield(S, f_t2)
        t2 = S.(f_t2);
        y_true = S.(sprintf('step2_%s_tau_x_true', tag));
        y_pred = S.(sprintf('step2_%s_tau_x_pred', tag));
        e2 = S.(sprintf('step2_%s_roll_error', tag));
        ...
        title(sprintf('%s: Step 2 Roll Control', lbl));
        xlabel('Time (s)'); ylabel('tau_x (N*m)'); grid on;
        legend('True', 'Pred', 'Train/Val', 'Val/Test', 'Location', 'best');
        saveas(f, fullfile(out_dir, sprintf('C64_%s_step2_roll_controls.png', lbl)));
        close(f);
        ...
        xline(t2(tr_end + 1), 'k--', 'LineWidth', 1);
        xline(t2(val_end + 1), 'k--', 'LineWidth', 1);
        title(sprintf('%s: Step 2 Roll Error Input', lbl));
        xlabel('Time (s)'); ylabel('roll error (rad)'); grid on;
        legend('error', 'Train/Val', 'Val/Test', 'Location', 'best');
        saveas(f, fullfile(out_dir, sprintf('C64_%s_step2_roll_error_inputs.png', lbl)));
        close(f);
    end
    ...
    if isfield(S, f_t3)
        ...
        plot(t3, S.(sprintf('step3_%s_model_phi', tag)), 'g', 'LineWidth', 1);
        plot(t3, S.(sprintf('step3_%s_phi_ref', tag)), 'r--', 'LineWidth', 1);
        title(sprintf('%s: Roll tracking', lbl));
        ...
        plot(t3, S.(sprintf('step3_%s_model_tau_x', tag)), 'g', 'LineWidth', 1);
        title('Roll control'); xlabel('Time (s)'); ylabel('tau_x (N*m)'); grid on;
        ...
        saveas(f, fullfile(out_dir, sprintf('C64_%s_step3_pid_vs_model.png', lbl)));
        close(f);
    end
end
