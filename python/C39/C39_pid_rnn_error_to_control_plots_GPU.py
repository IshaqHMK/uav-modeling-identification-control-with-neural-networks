"""
C33 plotting companion: diagnostics for the shared GRU PID model.

Loads the checkpoint produced by `C31_pid_rnn_gru_error_to_control_train.py` and
produces diagnostic figures (holdout/full controls, error traces, learning
curve) for each dataset plus the held-out test log.
"""
...
MODEL_PATH = os.path.join(HERE, "models", "C35_100shared_pid_gru_SL_50.pt")
SAVE_PREFIX = "C39_plots_"
...
def load_checkpoint(path: str):
    ...
    if any(key.startswith("gru.") for key in state_dict):
        remapped = {}
        for key, val in state_dict.items():
            remapped[("rnn." + key[len("gru."):]) if key.startswith("gru.") else key] = val
        state_dict = remapped
...
def main():
    ...
    print("\nFinished generating C33 plots. Close the windows to exit.")
    plt.show(block=True)
