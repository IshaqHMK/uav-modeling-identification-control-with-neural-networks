...
Step 2:
  - Load the direct model from C56 (state estimator).
  - Build indirect-controller GRU inputs from estimated states:
    [z_ref, z_hat, z_dot_hat, e_hat, e_hat_integral].
  - Split each dataset into train/val/test, normalize on train split,
    then train a GRU to predict U1 (PID target) from indirect features.

Step 3:
  - Use the trained indirect GRU controller in closed-loop on the same nonlinear plant.
  - The direct model provides z_hat and z_dot_hat online for controller features.
  - Compare tracking and control against fixed PID and report RMS error.
"""
...
# Figure saving
SAVE_PREFIX = "C57_WFdBk_"
FIG_DIR = os.path.dirname(os.path.abspath(__file__))
MODEL_SAVE_PREFIX = "C57_nonlinear_z_pid_WFdBk_indirectGRUmodel"
MODEL_SAVE_DIR = os.path.join(FIG_DIR, "models")

# Indirect method uses the direct model trained in C56.
DIRECT_MODEL_PREFIX = "C56_nonlinear_z_pid_WFdBk_directModel"
DIRECT_MODEL_PATH = os.path.join(MODEL_SAVE_DIR, f"{DIRECT_MODEL_PREFIX}_SL_{SEQUENCE_LENGTH}.pt")
...
def build_scaler_from_ckpt(scaler_dict):
    """Recreate a fitted StandardScaler from checkpoint data."""
...
class DirectStateEstimator(nn.Module):
    """Direct-model GRU cell used as state estimator (from C56)."""
...
def load_direct_model(device):
    """Load C56 direct model and its scalers."""
    if not os.path.exists(DIRECT_MODEL_PATH):
        raise FileNotFoundError(
            f"Direct-model checkpoint not found: {DIRECT_MODEL_PATH}. Train C56 first."
        )

    ckpt = torch.load(DIRECT_MODEL_PATH, map_location="cpu", weights_only=False)
    scaler_u = build_scaler_from_ckpt(ckpt["scaler_u"])
    scaler_y = build_scaler_from_ckpt(ckpt["scaler_y"])
    hidden_size = int(ckpt.get("training", {}).get("hidden_size", HIDDEN_SIZE))

    direct_model = DirectStateEstimator(3, hidden_size).to(device)
    direct_model.load_state_dict(ckpt["model_state"])
    direct_model.eval()
    return direct_model, scaler_u, scaler_y


def run_direct_estimator(u1_series, z_series, z_dot_series, direct_model, scaler_u, scaler_y):
    """Replay direct-model estimator over a trajectory to get z_hat and z_dot_hat."""
    n = len(u1_series)
    y_hat_hist = np.zeros((n, 2))
    model_device = next(direct_model.parameters()).device

    u_mean_t = torch.tensor(scaler_u.mean_.astype(np.float32), device=model_device)
    u_scale_t = torch.tensor(scaler_u.scale_.astype(np.float32), device=model_device)
    y_mean_t = torch.tensor(scaler_y.mean_.astype(np.float32), device=model_device)
    y_scale_t = torch.tensor(scaler_y.scale_.astype(np.float32), device=model_device)

    h = torch.zeros(1, direct_model.hidden_size, device=model_device)
    y0_scaled = (torch.tensor([[0.0, 0.0]], device=model_device) - y_mean_t) / y_scale_t
    y_hat_prev = y0_scaled

    for i in range(n):
        u_t = torch.tensor([[float(u1_series[i])]], dtype=torch.float32, device=model_device)
        y_true = torch.tensor([[float(z_series[i]), float(z_dot_series[i])]], dtype=torch.float32, device=model_device)

        y_true_scaled = (y_true - y_mean_t) / y_scale_t
        u_scaled = (u_t - u_mean_t) / u_scale_t
        err_scaled = y_true_scaled - y_hat_prev
        inp = torch.cat([u_scaled, err_scaled], dim=1)

        h = direct_model.cell(inp, h)
        y_hat_prev = direct_model.fc(h)
...
def build_indirect_features(ds, direct_model, scaler_u, scaler_y):
    """Build indirect-controller features from direct-model state estimates."""
    y_hat = run_direct_estimator(ds["u1"], ds["z"], ds["z_dot"], direct_model, scaler_u, scaler_y)
    z_hat = y_hat[:, 0]
    z_dot_hat = y_hat[:, 1]
    e_hat = ds["z_ref"] - z_hat
    e_hat_int = np.cumsum(e_hat) * Ts
    return np.column_stack([ds["z_ref"], z_hat, z_dot_hat, e_hat, e_hat_int])
...
def simulate_with_model(reference, model, scaler_X, scaler_Y, seq_len, Ts, direct_model, scaler_u, scaler_y, wind_force=0.0, wind_start_time=20.0):
    """Step 3: Run nonlinear plant with indirect GRU controller features."""
    z = 0.0
    z_dot = 0.0
    phi = 0.0
    theta = 0.0
    ...
    h_est = torch.zeros(1, direct_model.hidden_size, device=model_device)
    y_hat_prev = (torch.tensor([[0.0, 0.0]], device=model_device) - y_mean_t) / y_scale_t

    for i in range(n):
        z_ref = reference[i]
        y_hat = (y_hat_prev * y_scale_t) + y_mean_t
        z_hat = float(y_hat[0, 0].item())
        z_dot_hat = float(y_hat[0, 1].item())

        err_hat = z_ref - z_hat
        err_int += err_hat * Ts

        feature_hist.append([z_ref, z_hat, z_dot_hat, err_hat, err_int])

        if len(feature_hist) < seq_len:
            pad_len = seq_len - len(feature_hist)
            seq_data = np.vstack([np.zeros((pad_len, feature_dim)), np.array(feature_hist)])
        else:
            seq_data = np.array(feature_hist[-seq_len:])
        ...
        y_true = torch.tensor([[z, z_dot]], dtype=torch.float32, device=model_device)
        y_true_scaled = (y_true - y_mean_t) / y_scale_t
        u_t = torch.tensor([[u1]], dtype=torch.float32, device=model_device)
        u_scaled = (u_t - u_mean_t) / u_scale_t
        err_scaled = y_true_scaled - y_hat_prev
        est_inp = torch.cat([u_scaled, err_scaled], dim=1)
        with torch.no_grad():
            h_est = direct_model.cell(est_inp, h_est)
            y_hat_prev = direct_model.fc(h_est)
    ...
    return {"z": z_hist, "z_meas": z_meas_hist, "z_dot": z_dot_hist, "u1": u1_hist}
...
def main():
    ...
    print("\nStep 2: Training indirect GRU controller using C56 direct model")

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    direct_model, scaler_u, scaler_y = load_direct_model(device)
    print(f"Loaded direct model checkpoint: {DIRECT_MODEL_PATH}")
    ...
    for entry in datasets:
        ...
        features = build_indirect_features(ds, direct_model, scaler_u, scaler_y)
        targets = ds["u1"]
        ...
        error_seq = features[SEQUENCE_LENGTH - 1:, 3]
    ...
    ckpt = {
        ...
        "feature_names": ["z_ref", "z_hat", "z_dot_hat", "z_error_hat", "z_error_hat_integral"],
        "target_name": "u1_pid",
    ...
    print(f"Saved indirect-controller checkpoint to: {ckpt_path}")
    ...
    print("\nStep 3: Testing indirect GRU controller vs fixed PID")
    dataset_labels = [entry["label"] for entry in datasets]
    dataset_map = {entry["label"]: entry for entry in datasets}
    if PLOT_DATASET_LABEL == "ALL":
        plot_labels = dataset_labels
