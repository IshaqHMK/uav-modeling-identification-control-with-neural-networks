% C66 training MAT plotter (for SLURM headless runs)
% Loads C66_train_results.mat and regenerates C66 figures in MATLAB.

clear; clc;

base_dir = fileparts(mfilename('fullpath'));
mat_path = fullfile(base_dir, 'mat_results', 'C66_train_results.mat');
out_dir = fullfile(base_dir, 'c66_train_figures_matlab');
...
if isfield(S, 'step2_train_loss') && isfield(S, 'step2_validation_loss')
    ...
    xlabel('Epoch'); ylabel('MSE'); title('C66 Step 2 Yaw Learning Curve');
    grid on; legend({'Train', 'Validation'}, 'Location', 'best');
    saveas(f, fullfile(out_dir, 'C66_step2_yaw_learning_curve.png'));
    close(f);
end
...
for i = 1:numel(labels)
    ...
    if isfield(S, f_time)
        ...
        nexttile;
        plot(t, S.(sprintf('step1_%s_psi', tag)), 'Color', [0.9 0.4 0.1], 'LineWidth', 1); hold on;
        ...
        nexttile;
        plot(t, S.(sprintf('step1_%s_tau_z', tag)), 'LineWidth', 1);
        ...
        title('Roll/Pitch'); xlabel('Time (s)'); ylabel('rad'); grid on;
        legend('\phi','\phi_{ref}','\theta','\theta_{ref}', 'Location', 'best');

        nexttile;
        plot(t, S.(sprintf('step1_%s_wind', tag)), 'r', 'LineWidth', 1);
        title('Wind disturbance'); xlabel('Time (s)'); ylabel('N'); grid on;

        saveas(f, fullfile(out_dir, sprintf('C66_%s_step1_pid_dataset.png', lbl)));
        close(f);
    end
    ...
    if isfield(S, f_t2)
        t2 = S.(f_t2);
        y_true = S.(sprintf('step2_%s_tau_z_true', tag));
        y_pred = S.(sprintf('step2_%s_tau_z_pred', tag));
        e2 = S.(sprintf('step2_%s_yaw_error', tag));
        ...
        title(sprintf('%s: Step 2 Yaw Control', lbl));
        xlabel('Time (s)'); ylabel('tau_z (N*m)'); grid on;
        legend('True', 'Pred', 'Train/Val', 'Val/Test', 'Location', 'best');
        saveas(f, fullfile(out_dir, sprintf('C66_%s_step2_yaw_controls.png', lbl)));
        close(f);
        ...
        xline(t2(tr_end + 1), 'k--', 'LineWidth', 1);
        xline(t2(val_end + 1), 'k--', 'LineWidth', 1);
        title(sprintf('%s: Step 2 Yaw Error Input', lbl));
        xlabel('Time (s)'); ylabel('yaw error (rad)'); grid on;
        legend('error', 'Train/Val', 'Val/Test', 'Location', 'best');
        saveas(f, fullfile(out_dir, sprintf('C66_%s_step2_yaw_error_inputs.png', lbl)));
        close(f);
    end
    ...
    if isfield(S, f_t3)
        ...
        plot(t3, S.(sprintf('step3_%s_model_psi', tag)), 'g', 'LineWidth', 1);
        plot(t3, S.(sprintf('step3_%s_psi_ref', tag)), 'r--', 'LineWidth', 1);
        title(sprintf('%s: Yaw tracking', lbl));
        xlabel('Time (s)'); ylabel('psi (rad)'); grid on;
        legend('PID', 'GRU', 'ref', 'Location', 'best');

        nexttile;
        plot(t3, S.(sprintf('step3_%s_pid_tau_z', tag)), 'b', 'LineWidth', 1); hold on;
        plot(t3, S.(sprintf('step3_%s_model_tau_z', tag)), 'g', 'LineWidth', 1);
        title('Yaw control'); xlabel('Time (s)'); ylabel('tau_z (N*m)'); grid on;
        ...
        saveas(f, fullfile(out_dir, sprintf('C66_%s_step3_pid_vs_model.png', lbl)));
        close(f);
    end
end
