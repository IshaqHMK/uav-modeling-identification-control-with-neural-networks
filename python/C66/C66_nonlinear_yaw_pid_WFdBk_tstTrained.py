#!/usr/bin/env python3
"""
Test the trained C66 yaw-axis GRU controller on unseen yaw amplitudes.
...
SEQUENCE_LENGTH = c66.SEQUENCE_LENGTH
MODEL_SAVE_PREFIX = "C66_nonlinear_yaw_pid_WFdBk_trainedGRUmodel"
MODEL_SAVE_DIR = c66.MODEL_SAVE_DIR
MODEL_LOAD_PATH = os.path.join(MODEL_SAVE_DIR, f"{MODEL_SAVE_PREFIX}_SL_{SEQUENCE_LENGTH}.pt")

# Unseen yaw scales; keep highest disturbance by default.
TEST_EXPERIMENT_SPECS = [
    {"seed": 101, "wind_force": c66.MAX_WIND_FORCE, "yaw_amp_scale": 0.7},
    {"seed": 102, "wind_force": c66.MAX_WIND_FORCE, "yaw_amp_scale": 1.9},
]
...
MAT_SAVE_NAME = "C66_test_results.mat"
...
def main():
    print("\nStep 1: Build test PID datasets (yaw-focus)")
    ...
    mat_data = {
        ...
        "model_tag": np.array(["C66_yaw_test"], dtype=object),
        "config_Ts": np.array([c66.Ts], dtype=float),
        "config_total_time": np.array([c66.TOTAL_TIME], dtype=float),
        "config_num_samples": np.array([c66.NUM_SAMPLES], dtype=np.int32),
        ...
        "config_max_wind_force": np.array([c66.MAX_WIND_FORCE], dtype=float),
    }
    ...
    for cfg in TEST_EXPERIMENT_SPECS:
        ...
        label = f"TST_{c66.build_dataset_label(wind_force, ref_seed, yaw_amp_scale)}"
        ds = c66.run_nonlinear_pid(
            wind_force=wind_force,
            wind_start_time=c66.WIND_START_TIME,
            wind_end_time=c66.WIND_END_TIME,
            ref_seed=ref_seed,
        ...
        rms = float(np.sqrt(np.mean((ds["psi_ref"] - ds["psi"]) ** 2)))
        print(f"{label}: PID RMS yaw error = {rms:.4e} rad")
        if ENABLE_PYTHON_PLOTS:
            c66.plot_dataset(ds, label)
    ...
    model = YawGRURegressor(
        feature_dim,
        train_cfg.get("hidden_size", c66.HIDDEN_SIZE),
        1,
        num_layers=train_cfg.get("num_layers", c66.NUM_LAYERS),
        dropout=train_cfg.get("dropout", c66.DROPOUT),
    ).to(device)
    model.load_state_dict(ckpt["model_state"])
    model.eval()
    print(f"Loaded yaw GRU checkpoint: {MODEL_LOAD_PATH}")
    ...
    for entry in datasets:
        ...
        targets = ds["tau_z"]
        X_seq, Y_seq = c66.build_sequences(features, targets, seq_len)
        _, _, _, split_idx = c66.split_dataset(X_seq, Y_seq)
    ...
    for label in plot_labels:
        ...
        error_seq = ds["yaw_error"][seq_len - 1:]
        ...
        if ENABLE_PYTHON_PLOTS:
            c66.plot_yaw_results(label, time_seq, Y_seq, preds, error_seq, split_map[label])
    ...
    print("\nStep 3: PID vs yaw-GRU closed-loop test")
    yaw_artifacts = {
        "model": model,
        "scaler_X": scaler_X,
        "scaler_Y": scaler_Y,
    ...
    for label in plot_labels:
        ...
        ds = entry["data"]
        model_run = c66.simulate_with_model(
            ds["z_ref"],
            yaw_artifacts,
            seq_len,
            c66.Ts,
        ...
        if ENABLE_PYTHON_PLOTS:
            c66.plot_pid_vs_model(label, ds["time"], ds, model_run)
