#!/usr/bin/env python3
"""
C66: Nonlinear quadcopter experiment focused on yaw-axis GRU imitation.
...
Step 1:
  ...
  - Generate one training dataset (current yaw amplitude) and one validation
    dataset (different yaw amplitude).

Step 2:
  - Train one GRU for yaw axis only.
  - Inputs: [psi_meas, yaw_error, yaw_error_rate, yaw_error_integral]
  - Target: tau_z (yaw PID output)

Step 3:
  - Replace only yaw PID with trained GRU.
  - Keep z, roll, pitch on PID.
  - Compare yaw tracking and control effort against PID baseline.
...
# Disturbance setup: use highest wind value for all C66 PID simulations.
...
# Exactly one train dataset and one validation dataset for yaw tuning.
TRAIN_EXPERIMENT_SPECS = [
    {"seed": 21, "wind_force": MAX_WIND_FORCE, "yaw_amp_scale": 1.0},
]
VALIDATION_EXPERIMENT_SPECS = [
    {"seed": 21, "wind_force": MAX_WIND_FORCE, "yaw_amp_scale": 0.1},
]

PLOT_DATASET_LABEL = "ALL"

SAVE_PREFIX = "C66_WFdBk_"
FIG_DIR = os.path.dirname(os.path.abspath(__file__))
MODEL_SAVE_PREFIX = "C66_nonlinear_yaw_pid_WFdBk_trainedGRUmodel"
MODEL_SAVE_DIR = os.path.join(FIG_DIR, "models")
MAT_SAVE_DIR = os.path.join(FIG_DIR, "mat_results")
MAT_SAVE_NAME = "C66_train_results.mat"
...
def build_dataset_label(wind_force, ref_seed, yaw_amp_scale):
    return f"S{ref_seed}_W{format_value(wind_force)}_Y{format_value(yaw_amp_scale)}"
...
def attitude_references(t_now, yaw_amp_scale=1.0):
...
def run_nonlinear_pid(wind_force=0.0, wind_start_time=20.0, wind_end_time=None, ref_seed=None, amp_levels=None, yaw_amp_scale=1.0):
    ...
    for i in range(NUM_SAMPLES):
        ...
        phi_ref, theta_ref, psi_ref = attitude_references(t_now, yaw_amp_scale=yaw_amp_scale)
...
def plot_dataset(dataset, dataset_label):
    ...
    axs[1, 0].plot(time, dataset["psi"], label="psi", linewidth=1, color="tab:orange")
    axs[1, 0].plot(time, dataset["psi_ref"], "--", label="psi_ref", linewidth=1, color="k")
    axs[1, 0].set_title("Yaw (state)", fontsize=11)
    axs[1, 0].set_xlabel("Time (s)")
    axs[1, 0].set_ylabel("rad")
    axs[1, 0].grid(True, linestyle="--", alpha=0.7)
    axs[1, 0].legend(fontsize=8)

    axs[1, 1].plot(time, dataset["tau_z"], label="tau_z", linewidth=1, color="tab:red")
    axs[1, 1].set_title("Yaw control tau_z (PID)", fontsize=11)
    axs[1, 1].set_xlabel("Time (s)")
    axs[1, 1].set_ylabel("N*m")
    axs[1, 1].grid(True, linestyle="--", alpha=0.7)

    axs[2, 0].plot(time, dataset["phi"], label="phi", linewidth=1, color="m")
    axs[2, 0].plot(time, dataset["phi_ref"], "--", label="phi_ref", linewidth=1, color="k")
    axs[2, 0].plot(time, dataset["theta"], label="theta", linewidth=1, color="c")
    axs[2, 0].plot(time, dataset["theta_ref"], "--", label="theta_ref", linewidth=1, color="tab:gray")
    axs[2, 0].set_title("Roll/Pitch (PID only)", fontsize=11)
...
class YawGRURegressor(nn.Module):
...
def plot_learning_curve(history):
    ...
    plt.grid(alpha=0.3)
    plt.legend()
    plt.title("GRU learning curve (yaw axis)")
    plt.tight_layout()
    fig.savefig(os.path.join(FIG_DIR, f"{SAVE_PREFIX}step2_yaw_learning_curve.png"), dpi=300)
    plt.close(fig)


def plot_yaw_results(dataset_label, time_seq, y_true, y_pred, error_seq, split_idx):
    ...
    plt.xlabel("Time [s]")
    plt.suptitle(f"{dataset_label}: Yaw control (Train/Val/Test)")
    plt.tight_layout(rect=[0, 0.03, 1, 0.97])
    fig.savefig(os.path.join(FIG_DIR, f"{SAVE_PREFIX}{dataset_label}_step2_yaw_controls.png"), dpi=300)
    plt.close(fig)
    ...
    plt.grid(alpha=0.3)
    plt.xlabel("Time [s]")
    plt.suptitle(f"{dataset_label}: Yaw error input")
    plt.tight_layout(rect=[0, 0.03, 1, 0.97])
    fig.savefig(os.path.join(FIG_DIR, f"{SAVE_PREFIX}{dataset_label}_step2_yaw_error_inputs.png"), dpi=300)
    plt.close(fig)


def simulate_with_model(reference_z, yaw_artifacts, seq_len, Ts, wind_force=0.0, wind_start_time=20.0, wind_end_time=None, yaw_amp_scale=1.0):
    model = yaw_artifacts["model"]
    scaler_X = yaw_artifacts["scaler_X"]
    scaler_Y = yaw_artifacts["scaler_Y"]
    feature_dim = yaw_artifacts["feature_dim"]
    model_device = yaw_artifacts["device"]
    ...
    for i in range(n):
        ...
        # Yaw axis uses GRU instead of PID.
        yaw_buf["meas"].append(psi)
        yaw_buf["err"].append(yaw_err)
        yaw_buf["rate"].append(yaw_err_dot)
        yaw_buf["int"].append(yaw_int)

        hist_len = len(yaw_buf["err"])
        if hist_len < seq_len:
            pad_len = seq_len - hist_len
            seq_meas = np.concatenate([np.zeros(pad_len), np.array(yaw_buf["meas"])])
            seq_err = np.concatenate([np.zeros(pad_len), np.array(yaw_buf["err"])])
            seq_rate = np.concatenate([np.zeros(pad_len), np.array(yaw_buf["rate"])])
            seq_int = np.concatenate([np.zeros(pad_len), np.array(yaw_buf["int"])])
        else:
            seq_meas = np.array(yaw_buf["meas"][-seq_len:])
            seq_err = np.array(yaw_buf["err"][-seq_len:])
            seq_rate = np.array(yaw_buf["rate"][-seq_len:])
            seq_int = np.array(yaw_buf["int"][-seq_len:])
        ...
        tau_z = float(scaler_Y.inverse_transform(pred_scaled)[0, 0])
        tau_z = float(np.clip(tau_z, -YAW_TAU_CLIP, YAW_TAU_CLIP))
        # tau_x/tau_y remain PID while tau_z comes from GRU.
...
def plot_pid_vs_model(dataset_label, time, pid_data, model_data):
    ...
    axs[0, 0].plot(time, pid_data["psi"], label="PID psi", linewidth=1, color="b")
    axs[0, 0].plot(time, model_data["psi"], label="GRU psi", linewidth=1, color="g")
    axs[0, 0].plot(time, pid_data["psi_ref"], "--", label="psi_ref", linewidth=1, color="r")
    axs[0, 0].set_title(f"{dataset_label}: Yaw tracking")
    axs[0, 0].set_xlabel("Time (s)")
    axs[0, 0].set_ylabel("psi (rad)")
    axs[0, 0].grid(True, linestyle="--", alpha=0.7)
    axs[0, 0].legend(fontsize=8)

    axs[0, 1].plot(time, pid_data["tau_z"], label="PID tau_z", linewidth=1, color="b")
    axs[0, 1].plot(time, model_data["tau_z"], label="GRU tau_z", linewidth=1, color="g")
    axs[0, 1].set_title("Yaw control")
    axs[0, 1].set_xlabel("Time (s)")
...
def main():
    print("\nStep 1: Nonlinear PID datasets (yaw-focus)")
    print(f"Using highest disturbance for C66: {MAX_WIND_FORCE} N")

    train_datasets = []
    val_datasets = []
    mat_data = {
        "run_type": np.array(["train"], dtype=object),
        "model_tag": np.array(["C66_yaw_from_C62"], dtype=object),
        ...
        "config_yaw_train_amp_scale": np.array([TRAIN_EXPERIMENT_SPECS[0]["yaw_amp_scale"]], dtype=float),
        "config_yaw_val_amp_scale": np.array([VALIDATION_EXPERIMENT_SPECS[0]["yaw_amp_scale"]], dtype=float),
    }
    ...
    for cfg in TRAIN_EXPERIMENT_SPECS:
        ...
        label = f"TRN_{build_dataset_label(wind_force, ref_seed, yaw_amp_scale)}"
        ...
        print(f"{label}: RMS yaw error = {rms_yaw:.4e} rad")
    ...
    for cfg in VALIDATION_EXPERIMENT_SPECS:
        ...
        wind_force = float(cfg["wind_force"])
        yaw_amp_scale = float(cfg["yaw_amp_scale"])
        label = f"VAL_{build_dataset_label(wind_force, ref_seed, yaw_amp_scale)}"
        ds = run_nonlinear_pid(
            wind_force=wind_force,
            wind_start_time=WIND_START_TIME,
            wind_end_time=WIND_END_TIME,
    ...
    print("\nStep 2: Train GRU on yaw PID data")
    ...
    for entry in train_datasets:
        ds = entry["data"]
        features = np.column_stack([ds["psi"], ds["yaw_error"], ds["yaw_error_rate"], ds["yaw_error_int"]])
        targets = ds["tau_z"]
    ...
    model = YawGRURegressor(feature_dim, HIDDEN_SIZE, 1, num_layers=NUM_LAYERS, dropout=DROPOUT).to(device)
    ...
    ckpt = {
        ...
        "feature_names": ["psi", "yaw_error", "yaw_error_rate", "yaw_error_integral"],
        "target_name": "tau_z",
    ...
    print(f"Saved yaw GRU checkpoint to: {ckpt_path}")
    ...
    for label in plot_labels:
        ...
        error_seq = ds["yaw_error"][SEQUENCE_LENGTH - 1:]
        ...
        if ENABLE_PYTHON_PLOTS:
            plot_yaw_results(label, time_seq, Y_seq, preds, error_seq, split_map[label])

        tag = sanitize_label(label)
        split_idx = split_map[label]
        mat_data[f"step2_{tag}_time"] = time_seq
        mat_data[f"step2_{tag}_tau_z_true"] = Y_seq
        mat_data[f"step2_{tag}_tau_z_pred"] = preds
        mat_data[f"step2_{tag}_yaw_error"] = error_seq
    ...
    print("\nStep 3: Test yaw GRU vs fixed PID")
    model.eval()
    yaw_artifacts = {
        "model": model,
        "scaler_X": scaler_X,
    ...
    for label in plot_labels:
        ...
        pid_rms = float(np.sqrt(np.mean((ds["psi_ref"] - ds["psi"]) ** 2)))
        model_rms = float(np.sqrt(np.mean((ds["psi_ref"] - model_run["psi"]) ** 2)))
        print(f"{label} RMS yaw error | PID: {pid_rms:.4e} rad | Model: {model_rms:.4e} rad")
        ...
        mat_data[f"step3_{tag}_psi_ref"] = ds["psi_ref"]
        mat_data[f"step3_{tag}_pid_psi"] = ds["psi"]
        mat_data[f"step3_{tag}_pid_tau_z"] = ds["tau_z"]
        mat_data[f"step3_{tag}_model_psi"] = model_run["psi"]
        mat_data[f"step3_{tag}_model_tau_z"] = model_run["tau_z"]
        ...
        mat_data[f"step3_{tag}_pid_rms_yaw"] = np.array([pid_rms], dtype=float)
        mat_data[f"step3_{tag}_model_rms_yaw"] = np.array([model_rms], dtype=float)
