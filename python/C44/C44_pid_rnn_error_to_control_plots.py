...
MODEL_PATH = os.path.join(HERE, "models", "C43_ep50_shared_pid_gru_SL_10.pt")
SAVE_PREFIX = "C44_plots_"
...
def generate_plots_for_dataset(model, scaler_X, scaler_Y, sequence_length, dataset_label, mat_file):
    ...
    # Separate altitude vs attitude for unit conversion
    controls_true_full = Y_seq
    controls_pred_full = preds
    controls_true_holdout = Y_holdout
    controls_pred_holdout = preds_holdout
    # Convert attitude channels (u2-u4) to deg/s for plotting
    controls_true_full_deg = controls_true_full.copy()
    controls_pred_full_deg = controls_pred_full.copy()
    controls_true_holdout_deg = controls_true_holdout.copy()
    controls_pred_holdout_deg = controls_pred_holdout.copy()
    # indices 1,2,3 are attitude
    for arr in (controls_true_full_deg, controls_pred_full_deg, controls_true_holdout_deg, controls_pred_holdout_deg):
    ...
    # Convert attitude errors to deg; keep altitude in meters
    error_deg = error_aligned.copy()
    error_deg[:, 1:] = np.rad2deg(error_deg[:, 1:])
    plt.figure(figsize=(12, 6))
    for idx, label in enumerate(['z error [m]', 'phi error [deg]', 'theta error [deg]', 'psi error [deg]']):
        plt.subplot(4, 1, idx + 1)
        plt.plot(time_seq, error_deg[:, idx], linewidth=1)
        plt.ylabel(label)
        plt.grid(alpha=0.3)
    plt.xlabel('Time [s]')
    plt.suptitle(f"{dataset_label}: Error Inputs")
    plt.tight_layout(rect=[0, 0.03, 1, 0.97])
