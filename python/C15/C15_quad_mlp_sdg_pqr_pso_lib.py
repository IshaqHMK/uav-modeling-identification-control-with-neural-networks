# ================================================
# C15: MLP Direct Model (rad units) with PSO (library) init
# - Copy of C10 (direct one-step model in rad / rad/s)
# - Adds a PSO stage (via PySwarms) to initialize ONLY the final layer
...
# Optional PSO library (PySwarms)
try:
    from pyswarms.single import GlobalBestPSO
    _HAVE_PYSWARMS = True
except Exception:
    _HAVE_PYSWARMS = False
...
# PSO settings
USE_PSO_INIT = True       # set False to skip PSO
PSO_PARTICLES = 24
PSO_ITERS = 40
...
def main():
    ...
    # 3.1) PSO init (via PySwarms) — final layer only
    if USE_PSO_INIT and _HAVE_PYSWARMS:
        last = get_last_linear(model)
        W_shape, b_shape = last.weight.shape, last.bias.shape
        with torch.no_grad():
            base_W = last.weight.detach().clone()
            base_b = last.bias.detach().clone()
        base_vec = torch.cat([base_W.flatten(), base_b.flatten()]).cpu().numpy()
        dim = base_vec.size

        # Subset for quick objective
        n = X_train_t.shape[0]
        k = min(PSO_SUBSET, n)
        ...
        def objective(P):
            ...
            with torch.no_grad():
                for p in P:
                    pv = torch.tensor(p, dtype=torch.float32, device=device)
                    W_new = pv[:np.prod(W_shape)].view(W_shape)
                    b_new = pv[np.prod(W_shape):].view(b_shape)
                    last.weight.copy_(W_new)
                    last.bias.copy_(b_new)
                    y_pred = model(X_sub)
                    losses.append(nn.MSELoss()(y_pred, Y_sub).item())
            return np.array(losses)

        from pyswarms.single import GlobalBestPSO  # safe import here too
        options = {'w': PSO_W, 'c1': PSO_C1, 'c2': PSO_C2}
        optimizer = GlobalBestPSO(n_particles=PSO_PARTICLES, dimensions=dim, options=options, bounds=bounds)
        best_cost, best_pos = optimizer.optimize(objective, iters=PSO_ITERS)
        print(f"PSO best MSE (subset): {best_cost:.4e}")

        with torch.no_grad():
            pv = torch.tensor(best_pos, dtype=torch.float32, device=device)
            W_new = pv[:np.prod(W_shape)].view(W_shape)
            b_new = pv[np.prod(W_shape):].view(b_shape)
            last.weight.copy_(W_new)
            last.bias.copy_(b_new)
    elif USE_PSO_INIT and not _HAVE_PYSWARMS:
        print("[C15] PySwarms not installed; skipping PSO init. Install with: pip install pyswarms")

    # 4) Train with Adam (same as C10)
    ...
    fig1 = plt.figure(num="C15: Test-set Angles (True vs Pred)", figsize=PLOT_FIGSIZE)
    ...
    fig2 = plt.figure(num="C15: Test-set Rates (True vs Pred)", figsize=PLOT_FIGSIZE)
    ...
    fig3 = plt.figure(num="C15: Full Angles (True vs Pred)", figsize=PLOT_FIGSIZE)
    for i, label in enumerate(['phi [deg]', 'theta [deg]', 'psi [deg]']):
        plt.subplot(3,1,i+1)
    ...
    # Rates - full sequence
    fig4 = plt.figure(num="C15: Full Rates (True vs Pred)", figsize=PLOT_FIGSIZE)
    for i, label in enumerate(['p [deg/s]', 'q [deg/s]', 'r [deg/s]']):
        plt.subplot(3,1,i+1)
    ...
    # Learning curves
    fig5 = plt.figure(num="C15: Learning Curves", figsize=(6,4))
    ...
    ckpt = {
        ...
        "pso": {
            "used": USE_PSO_INIT and _HAVE_PYSWARMS,
