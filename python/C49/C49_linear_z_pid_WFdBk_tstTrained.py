...
WIND_FORCE = 4.0  # N (set to 0.0 to disable or change for unseen tests)
WIND_MODE = "prbs"  # "prbs", "constant", or "none"
WIND_SETTINGS = {
    "prbs": {
        "hold_steps": 100,
        "taps": (6, 5),
...
# Figure saving
SAVE_PREFIX = "C49_WFdBk_testTrained_"
FIG_DIR = os.path.dirname(os.path.abspath(__file__))
# Load the C49 WFdBk model trained with PRBS wind disturbance sweep.
MODEL_SAVE_PREFIX = "C49_linear_z_pid_WFdBk_trainedGRUmodel"
...
def apply_wind_disturbance(step_idx, wind_force, wind_state):
    """PRBS wind disturbance generator (test amplitude)."""
    mode = WIND_MODE.lower()
    if mode == "none" or wind_force == 0.0:
        return 0.0, wind_state
    if mode == "constant":
        return wind_force, wind_state
    if mode == "prbs":
        cfg = WIND_SETTINGS["prbs"]
        hold = cfg["hold_steps"]
        taps = cfg["taps"]
        width = cfg["width"]
        prbs_state = wind_state.get("prbs_state", 0b1111111)
        prbs_sign = wind_state.get("prbs_sign", 1)

        if step_idx % hold == 0:
            bit, prbs_state = prbs_step(prbs_state, taps=taps, width=width)
            prbs_sign = 1 if bit else -1

        wind_state.update({"prbs_state": prbs_state, "prbs_sign": prbs_sign})
        return prbs_sign * wind_force, wind_state
...
def simulate_linear_z(
    ...
    noise_state = {}
    wind_state = {}
    wind_start_idx = int(round(wind_start_time / Ts))
    ...
    for i in range(n):
        ...
        # PRBS disturbance applied only after the chosen time.
        if i >= wind_start_idx:
            wind, wind_state = apply_wind_disturbance(i - wind_start_idx, wind_force, wind_state)
        else:
            wind = 0.0
...
def main():
    ...
    for dataset_id, ds in zip(DATASET_IDS, datasets):
        ...
        split_map[f"D{dataset_id}"] = split_idx
