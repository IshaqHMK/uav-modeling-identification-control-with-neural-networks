% C68 test MAT plotter (for headless/HPC runs)
% Loads C68_test_results.mat and regenerates C68 comparison figures in MATLAB.

clear; clc;

base_dir = fileparts(mfilename('fullpath'));
mat_path = fullfile(base_dir, 'mat_results', 'C68_test_results.mat');
out_dir = fullfile(base_dir, 'c68_test_figures_matlab');
...
for i = 1:numel(labels)
    ...
    if isfield(S, f_t1)
        ...
        title('Roll (PID)'); xlabel('Time (s)'); ylabel('\phi (rad)'); grid on;
        ...
        title('Pitch (PID)'); xlabel('Time (s)'); ylabel('\theta (rad)'); grid on;
        ...
        title('Yaw (PID)'); xlabel('Time (s)'); ylabel('\psi (rad)'); grid on;
        ...
        title('\tau_z and wind (PID)'); xlabel('Time (s)'); ylabel('N*m / N'); grid on;
        legend('\tau_z', 'wind', 'Location', 'best');

        saveas(f, fullfile(out_dir, sprintf('C68_%s_step1_pid_dataset.png', lbl)));
        close(f);
    end

    % ---------------- Step 2 (joint model predictions) ----------------
    f_t2 = sprintf('step2_%s_time', tag);
    if isfield(S, f_t2)
        t2 = S.(f_t2);

        u1_true = S.(sprintf('step2_%s_u1_true', tag));
        u1_pred = S.(sprintf('step2_%s_u1_pred', tag));
        tx_true = S.(sprintf('step2_%s_tau_x_true', tag));
        tx_pred = S.(sprintf('step2_%s_tau_x_pred', tag));
        ty_true = S.(sprintf('step2_%s_tau_y_true', tag));
        ty_pred = S.(sprintf('step2_%s_tau_y_pred', tag));
        tz_true = S.(sprintf('step2_%s_tau_z_true', tag));
        tz_pred = S.(sprintf('step2_%s_tau_z_pred', tag));
        e2 = S.(sprintf('step2_%s_errors', tag));
        tr_end = S.(sprintf('step2_%s_split_train_end', tag)); tr_end = tr_end(1);
        val_end = S.(sprintf('step2_%s_split_val_end', tag)); val_end = val_end(1);

        f = figure('Visible', 'off', 'Color', 'w', 'Position', [100 100 1100 720]);
        tiledlayout(2, 2, 'Padding', 'compact', 'TileSpacing', 'compact');

        nexttile;
        plot(t2, u1_true, 'b', 'LineWidth', 1); hold on;
        plot(t2, u1_pred, 'r', 'LineWidth', 1);
        xline(t2(tr_end + 1), 'k--', 'LineWidth', 1);
        xline(t2(val_end + 1), 'k--', 'LineWidth', 1);
        title(sprintf('%s: u1', lbl));
        xlabel('Time (s)'); ylabel('N'); grid on;
        legend('True', 'Pred', 'Train/Val', 'Val/Test', 'Location', 'best');

        nexttile;
        plot(t2, tx_true, 'b', 'LineWidth', 1); hold on;
        plot(t2, tx_pred, 'r', 'LineWidth', 1);
        xline(t2(tr_end + 1), 'k--', 'LineWidth', 1);
        xline(t2(val_end + 1), 'k--', 'LineWidth', 1);
        title('\tau_x');
        xlabel('Time (s)'); ylabel('N*m'); grid on;
        legend('True', 'Pred', 'Train/Val', 'Val/Test', 'Location', 'best');

        nexttile;
        plot(t2, ty_true, 'b', 'LineWidth', 1); hold on;
        plot(t2, ty_pred, 'r', 'LineWidth', 1);
        xline(t2(tr_end + 1), 'k--', 'LineWidth', 1);
        xline(t2(val_end + 1), 'k--', 'LineWidth', 1);
        title('\tau_y');
        xlabel('Time (s)'); ylabel('N*m'); grid on;
        ...
        nexttile;
        plot(t2, tz_true, 'b', 'LineWidth', 1); hold on;
        plot(t2, tz_pred, 'r', 'LineWidth', 1);
        xline(t2(tr_end + 1), 'k--', 'LineWidth', 1);
        xline(t2(val_end + 1), 'k--', 'LineWidth', 1);
        title('\tau_z');
        xlabel('Time (s)'); ylabel('N*m'); grid on;
        ...
        saveas(f, fullfile(out_dir, sprintf('C68_%s_step2_joint_controls.png', lbl)));
        close(f);
        ...
        nexttile;
        plot(t2, e2(:, 1), 'k', 'LineWidth', 1); hold on;
        xline(t2(tr_end + 1), 'k--', 'LineWidth', 1);
        xline(t2(val_end + 1), 'k--', 'LineWidth', 1);
        title('z error'); xlabel('Time (s)'); ylabel('m'); grid on;
        legend('error', 'Train/Val', 'Val/Test', 'Location', 'best');

        nexttile;
        plot(t2, e2(:, 2), 'k', 'LineWidth', 1); hold on;
        xline(t2(tr_end + 1), 'k--', 'LineWidth', 1);
        xline(t2(val_end + 1), 'k--', 'LineWidth', 1);
        title('roll error'); xlabel('Time (s)'); ylabel('rad'); grid on;
        legend('error', 'Train/Val', 'Val/Test', 'Location', 'best');

        nexttile;
        plot(t2, e2(:, 3), 'k', 'LineWidth', 1); hold on;
        xline(t2(tr_end + 1), 'k--', 'LineWidth', 1);
        xline(t2(val_end + 1), 'k--', 'LineWidth', 1);
        title('pitch error'); xlabel('Time (s)'); ylabel('rad'); grid on;
        legend('error', 'Train/Val', 'Val/Test', 'Location', 'best');

        nexttile;
        plot(t2, e2(:, 4), 'k', 'LineWidth', 1); hold on;
        xline(t2(tr_end + 1), 'k--', 'LineWidth', 1);
        xline(t2(val_end + 1), 'k--', 'LineWidth', 1);
        title('yaw error'); xlabel('Time (s)'); ylabel('rad'); grid on;
        legend('error', 'Train/Val', 'Val/Test', 'Location', 'best');

        saveas(f, fullfile(out_dir, sprintf('C68_%s_step2_error_inputs.png', lbl)));
        close(f);
    end
    ...
    if isfield(S, f_t3)
        ...
        title(sprintf('%s: Altitude PID vs 1-GRU', lbl));
        xlabel('Time (s)'); ylabel('z (m)'); grid on;
        legend('PID', '1-GRU', 'z_{ref}', 'Location', 'best');

        nexttile;
        plot(t3, S.(sprintf('step3_%s_pid_u1', tag)), 'b', 'LineWidth', 1); hold on;
        plot(t3, S.(sprintf('step3_%s_gru_u1', tag)), 'g', 'LineWidth', 1);
        title('u1'); xlabel('Time (s)'); ylabel('N'); grid on;
        legend('PID', '1-GRU', 'Location', 'best');
        ...
        title('Roll'); xlabel('Time (s)'); ylabel('\phi (rad)'); grid on;
        legend('PID', '1-GRU', '\phi_{ref}', 'Location', 'best');

        nexttile;
        plot(t3, S.(sprintf('step3_%s_pid_tau_x', tag)), 'b', 'LineWidth', 1); hold on;
        plot(t3, S.(sprintf('step3_%s_gru_tau_x', tag)), 'g', 'LineWidth', 1);
        title('\tau_x'); xlabel('Time (s)'); ylabel('N*m'); grid on;
        legend('PID', '1-GRU', 'Location', 'best');
        ...
        title('Pitch'); xlabel('Time (s)'); ylabel('\theta (rad)'); grid on;
        legend('PID', '1-GRU', '\theta_{ref}', 'Location', 'best');

        nexttile;
        plot(t3, S.(sprintf('step3_%s_pid_tau_y', tag)), 'b', 'LineWidth', 1); hold on;
        plot(t3, S.(sprintf('step3_%s_gru_tau_y', tag)), 'g', 'LineWidth', 1);
        title('\tau_y'); xlabel('Time (s)'); ylabel('N*m'); grid on;
        legend('PID', '1-GRU', 'Location', 'best');
        ...
        title('Yaw'); xlabel('Time (s)'); ylabel('\psi (rad)'); grid on;
        legend('PID', '1-GRU', '\psi_{ref}', 'Location', 'best');

        nexttile;
        plot(t3, S.(sprintf('step3_%s_pid_tau_z', tag)), 'b', 'LineWidth', 1); hold on;
        plot(t3, S.(sprintf('step3_%s_gru_tau_z', tag)), 'g', 'LineWidth', 1);
        title('\tau_z'); xlabel('Time (s)'); ylabel('N*m'); grid on;
        legend('PID', '1-GRU', 'Location', 'best');

        saveas(f, fullfile(out_dir, sprintf('C68_%s_step3_pid_vs_1gru.png', lbl)));
        close(f);
        ...
        ylabel('RMS error');
        title(sprintf('%s: RMS comparison PID vs 1-GRU', lbl));
        grid on;
        legend('PID', '1-GRU', 'Location', 'best');
        saveas(f, fullfile(out_dir, sprintf('C68_%s_step3_rms_comparison.png', lbl)));
        close(f);
    end
end
