"""
C25 plotting companion: diagnostics for the shared vanilla-RNN PID model.

Loads the checkpoint produced by `C25_pid_rnn_error_to_control.py` and produces
diagnostic figures (holdout/full controls, error traces, learning curve) for
each dataset plus the held-out test log.
"""
...
MODEL_PATH = os.path.join(HERE, "models", "C25_shared_pid_rnn_SL_50.pt")
SAVE_PREFIX = "C25_plots_"

DATASET_CONFIGS = [
    ("D1", "quad_AGD__01_05_25_11_06_38.mat"),
    ("D2", "quad_AGD__06_05_25_10_34_54.mat"),
    ("D3", "quad_AGD__01_05_25_11_42_16.mat"),
    ("TEST", "quad_AGD__11_06_25_15_37_26.mat"),
]
...
class VanillaRNNRegressor(nn.Module):
    """Stacked vanilla RNN followed by a linear head predicting all control channels."""
...
def load_checkpoint(path: str):
    ...
    model = VanillaRNNRegressor(**model_kwargs)
...
def plot_learning_curve(train_history, val_history, save_path: str):
    ...
    plt.grid(alpha=0.3)
    plt.legend()
    plt.title("Vanilla RNN unified model learning curve")
    plt.tight_layout()
...
def main():
    print(f"Loading vanilla-RNN checkpoint: {MODEL_PATH}")
    ...
    print("\nFinished generating C25 plots. Close the windows to exit.")
    plt.show(block=True)
