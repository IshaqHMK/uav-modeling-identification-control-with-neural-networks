% C60 test MAT plotter (for SLURM headless runs)
% Loads C60_test_results.mat and regenerates C55-test-style figures in MATLAB.

clear; clc;

base_dir = fileparts(mfilename('fullpath'));
mat_path = fullfile(base_dir, 'mat_results', 'C60_test_results.mat');
out_dir = fullfile(base_dir, 'c60_test_figures_matlab');
...
for i = 1:numel(labels)
    ...
    if isfield(S, f_time)
        ...
        saveas(f, fullfile(out_dir, sprintf('C60_%s_step1_z_tracking.png', lbl)));
        close(f);

        % Step 1 extra: cos(phi)*cos(theta)
        if isfield(S, sprintf('step1_%s_cos_term', tag))
            f = figure('Visible', 'off', 'Color', 'w', 'Position', [100 100 900 360]);
            plot(t, S.(sprintf('step1_%s_cos_term', tag)), 'LineWidth', 1);
            title(sprintf('%s: cos(phi)*cos(theta)', lbl));
            xlabel('Time (s)'); ylabel('cos(phi)cos(theta)'); grid on;
            saveas(f, fullfile(out_dir, sprintf('C60_%s_step1_cos_term.png', lbl)));
            close(f);
        end

        % Step 1 extra: u1 noise
        if isfield(S, sprintf('step1_%s_u1_noise', tag))
            f = figure('Visible', 'off', 'Color', 'w', 'Position', [100 100 900 360]);
            plot(t, S.(sprintf('step1_%s_u1_noise', tag)), 'LineWidth', 1);
            title(sprintf('%s: U1 Noise', lbl));
            xlabel('Time (s)'); ylabel('Delta U1 (N)'); grid on;
            saveas(f, fullfile(out_dir, sprintf('C60_%s_step1_u1_noise.png', lbl)));
            close(f);
        end
    end
    ...
    if isfield(S, f_t2)
        ...
        saveas(f, fullfile(out_dir, sprintf('C60_%s_step2_controls.png', lbl)));
        close(f);
        ...
        saveas(f, fullfile(out_dir, sprintf('C60_%s_step2_error_inputs.png', lbl)));
        close(f);
    end
    ...
    if isfield(S, f_t3)
        ...
        plot(t3, mdl_z, 'r', 'LineWidth', 1);
        plot(t3, z_ref3, 'm--', 'LineWidth', 1);
        ...
        plot(t3, mdl_u, 'r', 'LineWidth', 1);
        title('Step 3 Control Input U1');
        xlabel('Time (s)'); ylabel('U1 (N)'); grid on;
        legend('PID U1', 'Model U1', 'Location', 'best');

        saveas(f, fullfile(out_dir, sprintf('C60_%s_step3_pid_vs_model.png', lbl)));
        close(f);
    end
end
