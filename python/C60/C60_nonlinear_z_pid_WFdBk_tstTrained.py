...
SAVE_PREFIX = "C60_WFdBk_testTrained_"
FIG_DIR = os.path.dirname(os.path.abspath(__file__))
# Load the C60 nonlinear model trained with the APRBS envelope reference.
MODEL_SAVE_PREFIX = "C60_nonlinear_z_pid_WFdBk_trainedGRUmodel"
...
MAT_SAVE_NAME = "C60_test_results.mat"
...
def plot_dataset(dataset, dataset_label):
    ...
    plt.grid(alpha=0.3)
    plt.tight_layout()
    fig.savefig(os.path.join(FIG_DIR, f"{SAVE_PREFIX}{dataset_label}_step1_cos_term.png"), dpi=300)
    ...
    plt.grid(alpha=0.3)
    plt.tight_layout()
    fig.savefig(os.path.join(FIG_DIR, f"{SAVE_PREFIX}{dataset_label}_step1_u1_noise.png"), dpi=300)
...
def plot_pid_vs_model(dataset_label, time, z_ref, pid_data, model_data):
    ...
    axs[0].plot(time, model_data["z"], label="Model z", linewidth=1, color="r")
    axs[0].plot(time, z_ref, "--", label="z_ref", linewidth=1, color="m")
    ...
    axs[1].plot(time, model_data["u1"], label="Model U1", linewidth=1, color="r")
...
def main():
    ...
    mat_data = {
        ...
        "model_tag": np.array(["C60_from_C55"], dtype=object),
        ...
        "config_reference_scale": np.array([REFERENCE_SCALE], dtype=float),
        "config_aprbs_seed": np.array([APRBS_RNG_SEED], dtype=np.int32),
    }
    ...
    mat_data["dataset_labels"] = np.array([ds["label"] for ds in datasets], dtype=object)
