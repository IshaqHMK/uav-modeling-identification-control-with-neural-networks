% C60 training MAT plotter (for SLURM headless runs)
% Loads C60_train_results.mat and regenerates C55-style figures in MATLAB.

clear; clc;

base_dir = fileparts(mfilename('fullpath'));
mat_path = fullfile(base_dir, 'mat_results', 'C60_train_results.mat');
out_dir = fullfile(base_dir, 'c60_train_figures_matlab');
...
if isfield(S, 'step2_train_loss') && isfield(S, 'step2_validation_loss')
    ...
    xlabel('Epoch');
    ylabel('MSE Loss');
    title('C60 Step 2 Learning Curve');
    grid on;
    legend({'Train', 'Validation'}, 'Location', 'best');
    saveas(f, fullfile(out_dir, 'C60_step2_learning_curve.png'));
    close(f);
end
