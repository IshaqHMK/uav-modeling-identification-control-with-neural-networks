#!/usr/bin/env python3
"""
Nonlinear quadcopter control experiment with one joint GRU controller.

Step 1:
  - Simulate nonlinear z/roll/pitch/yaw dynamics with fixed PID controllers.
  - Apply the A_env altitude reference plus gated attitude references.
  - Store multiaxis states, PID errors, and PID control channels.

Step 2:
  - Build one 16-feature GRU input from z/phi/theta/psi tracking signals.
  - Train a single GRU to predict the 4 controller outputs:
    [u1, tau_x, tau_y, tau_z].

Step 3:
  - Replace the four PID outputs with the trained GRU outputs.
  - Compare multiaxis tracking and control against the fixed PID baseline.
"""
...
#   - GRU output: [u1, tau_x, tau_y, tau_z] at time k
#   - Plant update applies these 4 control channels to the nonlinear dynamics
...
MAT_SAVE_NAME = "C61_train_results.mat"
...
def run_nonlinear_z_pid(wind_force=0.0, wind_start_time=20.0, aprbs_seed=None):
    """Step 1: Simulate nonlinear 4-axis dynamics with fixed PID."""
    ...
    roll_err_hist = np.zeros(NUM_SAMPLES)
    roll_err_rate_hist = np.zeros(NUM_SAMPLES)
    roll_err_int_hist = np.zeros(NUM_SAMPLES)
    pitch_err_hist = np.zeros(NUM_SAMPLES)
    pitch_err_rate_hist = np.zeros(NUM_SAMPLES)
    pitch_err_int_hist = np.zeros(NUM_SAMPLES)
    yaw_err_hist = np.zeros(NUM_SAMPLES)
    yaw_err_rate_hist = np.zeros(NUM_SAMPLES)
    yaw_err_int_hist = np.zeros(NUM_SAMPLES)
    tau_x_hist = np.zeros(NUM_SAMPLES)
    tau_y_hist = np.zeros(NUM_SAMPLES)
    tau_z_hist = np.zeros(NUM_SAMPLES)
    ...
    for i in range(NUM_SAMPLES):
        ...
        roll_err_hist[i] = roll_err
        roll_err_rate_hist[i] = roll_err_dot
        roll_err_int_hist[i] = roll_int
        pitch_err_hist[i] = pitch_err
        pitch_err_rate_hist[i] = pitch_err_dot
        pitch_err_int_hist[i] = pitch_int
        yaw_err_hist[i] = yaw_err
        yaw_err_rate_hist[i] = yaw_err_dot
        yaw_err_int_hist[i] = yaw_int
        tau_x_hist[i] = tau_x
        tau_y_hist[i] = tau_y
        tau_z_hist[i] = tau_z
        prev_err = err
    ...
    return {
        ...
        "roll_error": roll_err_hist,
        "roll_error_rate": roll_err_rate_hist,
        "roll_error_int": roll_err_int_hist,
        "pitch_error": pitch_err_hist,
        "pitch_error_rate": pitch_err_rate_hist,
        "pitch_error_int": pitch_err_int_hist,
        "yaw_error": yaw_err_hist,
        "yaw_error_rate": yaw_err_rate_hist,
        "yaw_error_int": yaw_err_int_hist,
        "tau_x": tau_x_hist,
        "tau_y": tau_y_hist,
        "tau_z": tau_z_hist,
    }
...
def build_multiaxis_features(dataset):
    """Build one 16-feature controller input from all 4 controlled axes."""
...
def build_multiaxis_targets(dataset):
    """Target controller channels learned from the fixed PID baseline."""
    return np.column_stack([
        dataset["u1"],
        dataset["tau_x"],
        dataset["tau_y"],
...
class ZRNNRegressor(nn.Module):
    """Step 2/3: GRU regressor for 4-axis control."""
...
def plot_learning_curve(history):
    ...
    plt.grid(alpha=0.3)
    plt.legend()
    plt.title("GRU learning curve (4-axis controller)")
    plt.tight_layout()
...
def plot_results(dataset_label, time_seq, y_true, y_pred, error_seq, split_idx):
    """Step 2 plot: 4 controller outputs and 4 tracked errors."""
    train_end, val_end = split_idx

    fig, axs = plt.subplots(2, 2, figsize=(11, 7))
    control_names = ["u1", "tau_x", "tau_y", "tau_z"]
    for idx, ax in enumerate(axs.ravel()):
        ...
        ax.set_title(control_names[idx])
    ...
    fig, axs = plt.subplots(2, 2, figsize=(11, 7))
    error_names = ["z error", "phi error", "theta error", "psi error"]
    for idx, ax in enumerate(axs.ravel()):
        ax.plot(time_seq, error_seq[:, idx], linewidth=1)
        ax.axvline(time_seq[train_end], color="k", linestyle=":", linewidth=1)
        ax.axvline(time_seq[val_end], color="k", linestyle="--", linewidth=1)
        ax.set_title(error_names[idx])
        ax.set_xlabel("Time [s]")
        ax.grid(alpha=0.3)
...
def simulate_with_model(reference, model, scaler_X, scaler_Y, seq_len, Ts, wind_force=0.0, wind_start_time=20.0):
    """Step 3: Run nonlinear dynamics using the joint 4-output GRU controller."""
    z = 0.0
    z_dot = 0.0
    phi = 0.0
    theta = 0.0
    ...
    u1_hist = np.zeros(n)
    tau_x_hist = np.zeros(n)
    tau_y_hist = np.zeros(n)
    tau_z_hist = np.zeros(n)
    phi_hist = np.zeros(n)
    ...
    for i in range(n):
        ...
        z_meas = z
        z_err = z_ref - z_meas
        z_err_rate = 0.0 if i == 0 else (z_err - z_err_hist[-1]) / Ts
        error_int += z_err * Ts
        ...
        phi_meas_list.append(phi)
        theta_meas_list.append(theta)
        psi_meas_list.append(psi)
        z_err_hist.append(z_err)
        z_err_rate_hist.append(z_err_rate)
        z_err_int_hist.append(error_int)
        phi_err_hist.append(roll_err)
        phi_err_rate_hist.append(roll_err_dot)
        phi_err_int_hist.append(roll_int)
        theta_err_hist.append(pitch_err)
        theta_err_rate_hist.append(pitch_err_dot)
        theta_err_int_hist.append(pitch_int)
        psi_err_hist.append(yaw_err)
        psi_err_rate_hist.append(yaw_err_dot)
        psi_err_int_hist.append(yaw_int)

        if len(z_err_hist) < seq_len:
            pad_len = seq_len - len(z_err_hist)
            feature_stack = np.vstack([
                np.zeros((pad_len, feature_dim)),
                np.column_stack([
                    np.array(z_meas_list),
                    np.array(z_err_hist),
                    np.array(z_err_rate_hist),
                    np.array(z_err_int_hist),
                    np.array(phi_meas_list),
                    np.array(phi_err_hist),
                    np.array(phi_err_rate_hist),
                    np.array(phi_err_int_hist),
                    np.array(theta_meas_list),
                    np.array(theta_err_hist),
                    np.array(theta_err_rate_hist),
                    np.array(theta_err_int_hist),
                    np.array(psi_meas_list),
                    np.array(psi_err_hist),
                    np.array(psi_err_rate_hist),
                    np.array(psi_err_int_hist),
                ]),
            ])
        else:
            feature_stack = np.column_stack([
                np.array(z_meas_list[-seq_len:]),
                np.array(z_err_hist[-seq_len:]),
                np.array(z_err_rate_hist[-seq_len:]),
                np.array(z_err_int_hist[-seq_len:]),
                np.array(phi_meas_list[-seq_len:]),
                np.array(phi_err_hist[-seq_len:]),
                np.array(phi_err_rate_hist[-seq_len:]),
                np.array(phi_err_int_hist[-seq_len:]),
                np.array(theta_meas_list[-seq_len:]),
                np.array(theta_err_hist[-seq_len:]),
                np.array(theta_err_rate_hist[-seq_len:]),
                np.array(theta_err_int_hist[-seq_len:]),
                np.array(psi_meas_list[-seq_len:]),
                np.array(psi_err_hist[-seq_len:]),
                np.array(psi_err_rate_hist[-seq_len:]),
                np.array(psi_err_int_hist[-seq_len:]),
            ])
        ...
        wind = wind_force if t_now >= wind_start_time else 0.0
        f_wz = -wind  # keep same sign convention as earlier linear model
        # Plant update: apply the predicted 4 controller outputs to the nonlinear plant.
        z_ddot = (u1 * np.cos(phi) * np.cos(theta) - Kdz * z_dot + f_wz - m * g) / m
        z_dot += z_ddot * Ts
        z += z_dot * Ts
        phi_dot += phi_ddot * Ts
...
def plot_pid_vs_model(dataset_label, time, pid_data, model_data):
    """Step 3 plot: Compare PID vs trained joint controller on all axes."""
    fig, axs = plt.subplots(2, 2, figsize=(11, 7))
    specs = [
        ("z", "z_ref", "z"),
        ("phi", "phi_ref", "phi"),
    ...
    for ax, (state_key, ref_key, title) in zip(axs.ravel(), specs):
        ax.plot(time, pid_data[state_key], label="PID", linewidth=1)
        ax.plot(time, model_data[state_key], label="Model", linewidth=1)
        ax.plot(time, pid_data[ref_key], "--", label="ref", linewidth=1)
        ax.set_title(title)
        ax.set_xlabel("Time (s)")
        ax.grid(alpha=0.3)
    axs[0, 0].legend(fontsize=8)
    plt.suptitle(f"{dataset_label}: State Tracking")
    plt.tight_layout(rect=[0, 0.03, 1, 0.97])
    fig.savefig(os.path.join(FIG_DIR, f"{SAVE_PREFIX}{dataset_label}_step3_tracking.png"), dpi=300)

    fig, axs = plt.subplots(2, 2, figsize=(11, 7))
    control_specs = [("u1", "u1"), ("tau_x", "tau_x"), ("tau_y", "tau_y"), ("tau_z", "tau_z")]
    for ax, (key, title) in zip(axs.ravel(), control_specs):
        ax.plot(time, pid_data[key], label="PID", linewidth=1)
        ax.plot(time, model_data[key], label="Model", linewidth=1)
        ax.set_title(title)
        ax.set_xlabel("Time (s)")
        ax.grid(alpha=0.3)
    axs[0, 0].legend(fontsize=8)
    plt.suptitle(f"{dataset_label}: Control Outputs")
    plt.tight_layout(rect=[0, 0.03, 1, 0.97])
    fig.savefig(os.path.join(FIG_DIR, f"{SAVE_PREFIX}{dataset_label}_step3_controls.png"), dpi=300)
...
def main():
    ...
    print("\nStep 1: Nonlinear 4-axis PID test")
    ...
    mat_data = {
        ...
        "config_aprbs_seeds": np.array(APRBS_SEEDS, dtype=np.int32),
        "config_wind_levels": np.array(WIND_LEVELS, dtype=float),
    }
    for aprbs_seed in APRBS_SEEDS:
        for wind_force in WIND_FORCES:
            ...
            mat_data[f"step1_{tag}_aprbs_seed"] = np.array([aprbs_seed], dtype=np.int32)
            mat_data[f"step1_{tag}_rms_error"] = np.array([rms], dtype=float)


    # ------------------------ Step 2: Train one GRU on 4-axis PID data ------------------------ #
    print("\nStep 2: Training GRU on joint z/phi/theta/psi PID data")
    ...
    for entry in datasets:
        ...
        train_split, val_split, _, split_idx = split_dataset(X_seq, Y_seq)
    ...
    # Train GRU: minimize joint MSE between predicted and true [u1, tau_x, tau_y, tau_z].
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    target_dim = Y_train_all.shape[1]
    model = ZRNNRegressor(feature_dim, HIDDEN_SIZE, target_dim, num_layers=NUM_LAYERS, dropout=DROPOUT).to(device)
    ...
    ckpt = {
        ...
        "feature_names": [
            "z_meas", "z_error", "z_error_rate", "z_error_integral",
            "phi_meas", "phi_error", "phi_error_rate", "phi_error_integral",
            "theta_meas", "theta_error", "theta_error_rate", "theta_error_integral",
            "psi_meas", "psi_error", "psi_error_rate", "psi_error_integral",
        ],
    ...
    # ------------------------ Step 3: Test trained joint model vs fixed PID ------------------------ #
    print("\nStep 3: Testing trained joint model vs fixed PID")
    dataset_labels = [entry["label"] for entry in datasets]
    dataset_map = {entry["label"]: entry for entry in datasets}
    if PLOT_DATASET_LABEL == "ALL":
        plot_labels = dataset_labels
