% C61 training MAT plotter (for SLURM headless runs)
% Loads C61_train_results.mat and regenerates C59-style figures in MATLAB.

clear; clc;

base_dir = fileparts(mfilename('fullpath'));
mat_path = fullfile(base_dir, 'mat_results', 'C61_train_results.mat');
out_dir = fullfile(base_dir, 'c61_train_figures_matlab');
...
if isfield(S, 'step2_train_loss') && isfield(S, 'step2_validation_loss')
    ...
    xlabel('Epoch');
    ylabel('MSE Loss');
    title('C61 Step 2 Learning Curve');
    grid on;
    legend({'Train', 'Validation'}, 'Location', 'best');
    saveas(f, fullfile(out_dir, 'C61_step2_learning_curve.png'));
    close(f);
end
