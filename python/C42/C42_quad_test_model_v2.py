...
# Model/test data paths
MODEL_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "models", "C43_shared_pid_gru_SL_50.pt")
DATASET_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "C43_sim_dataset_1.mat")
...
# ----------------------------- Simulation Main ----------------------------- #
def main():
    # Load trained model and dataset (PID baseline + references)
    ...
    control_inputs_history = np.zeros((num_samples, 4))  # model outputs
    motor_speeds_history = np.zeros((num_samples, 4))
    reference_history = reference_history_ds[:num_samples].copy()
    rate_reference_history = np.zeros((num_samples, 3))  # unused here but kept for structure
    noise_state = {}
    ...
    for i in range(num_samples):
        ...
        # Error signals (matches training)
        ...
        # Controller: warm-up with baseline PID controls until seq_len is reached
        if len(error_hist) < seq_len:
            U1, U2, U3, U4 = baseline_controls[i]
        else:
        ...
        # Inject disturbance after controller, before saturation/dynamics
