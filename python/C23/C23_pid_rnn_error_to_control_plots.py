"""
C23 plotting companion: diagnostics for the unified PID RNN.

Loads the shared-model checkpoint produced by `C23_pid_rnn_error_to_control.py`
and recreates the standard control/feature/learning-curve plots for each
dataset, including the held-out test log.
"""
...
MODEL_PATH = os.path.join(HERE, "models", "C23_shared_pid_rnn_SL_50.pt")
SAVE_PREFIX = "C23_plots_"

DATASET_CONFIGS = [
    ("D1", "quad_AGD__11_06_25_15_37_26.mat"),
    ("D2", "quad_AGD__06_05_25_10_34_54.mat"),
    ("D3", "quad_AGD__01_05_25_11_42_16.mat"),
    ("TEST", "quad_AGD__01_05_25_11_06_38.mat"),
]
...
class RNNRegressor(nn.Module):
    """Stacked LSTM followed by a linear head that predicts all control channels."""
...
def evaluate_dataset(model: nn.Module, X: np.ndarray, scaler_Y: StandardScaler, sequence_length: int) -> np.ndarray:
    model.eval()
    preds_scaled = []
    with torch.no_grad():
...
def generate_plots_for_dataset(model, scaler_X, scaler_Y, sequence_length, dataset_label, mat_file):
    ...
    preds = evaluate_dataset(model, X_seq_s, scaler_Y, sequence_length)
...
def plot_learning_curve(train_history, val_history, save_path: str):
    ...
    plt.grid(alpha=0.3)
    plt.legend()
    plt.title("Unified Model Learning Curve")
    plt.tight_layout()
...
def main():
    ...
    print("\nFinished generating C23 plots. Close the windows to exit.")
    plt.show(block=True)
