% C62 test MAT plotter (for SLURM headless runs)
% Loads C62_test_results.mat and regenerates C62 test figures in MATLAB.

clear; clc;

base_dir = fileparts(mfilename('fullpath'));
mat_path = fullfile(base_dir, 'mat_results', 'C62_test_results.mat');
out_dir = fullfile(base_dir, 'c62_test_figures_matlab');
...
for i = 1:numel(labels)
    ...
    if isfield(S, f_time)
        ...
        if isfield(S, sprintf('step1_%s_cos_term', tag))
            ...
            saveas(f, fullfile(out_dir, sprintf('C62_%s_step1_cos_term.png', lbl)));
            close(f);
        end
        ...
        if isfield(S, sprintf('step1_%s_u1_noise', tag))
            ...
            saveas(f, fullfile(out_dir, sprintf('C62_%s_step1_u1_noise.png', lbl)));
            close(f);
        end
    end
