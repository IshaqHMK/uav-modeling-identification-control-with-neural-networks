#!/usr/bin/env python3
"""
C65: Nonlinear quadcopter experiment focused on pitch-axis GRU imitation.
...
Step 1:
  ...
  - Generate one training dataset (current pitch amplitude) and one validation
    dataset (different pitch amplitude).

Step 2:
  - Train one GRU for pitch axis only.
  - Inputs: [theta_meas, pitch_error, pitch_error_rate, pitch_error_integral]
  - Target: tau_y (pitch PID output)

Step 3:
  - Replace only pitch PID with trained GRU.
  - Keep z, roll, yaw on PID.
  - Compare pitch tracking and control effort against PID baseline.
...
# Disturbance setup: use highest wind value for all C65 PID simulations.
...
# Exactly one train dataset and one validation dataset for pitch tuning.
TRAIN_EXPERIMENT_SPECS = [
    {"seed": 21, "wind_force": MAX_WIND_FORCE, "pitch_amp_scale": 1.0},
]
VALIDATION_EXPERIMENT_SPECS = [
    {"seed": 21, "wind_force": MAX_WIND_FORCE, "pitch_amp_scale": 0.1},
]

PLOT_DATASET_LABEL = "ALL"

SAVE_PREFIX = "C65_WFdBk_"
...
MAT_SAVE_NAME = "C65_train_results.mat"
...
def build_dataset_label(wind_force, ref_seed, pitch_amp_scale):
    return f"S{ref_seed}_W{format_value(wind_force)}_P{format_value(pitch_amp_scale)}"
...
def attitude_references(t_now, pitch_amp_scale=1.0):
...
def run_nonlinear_pid(wind_force=0.0, wind_start_time=20.0, wind_end_time=None, ref_seed=None, amp_levels=None, pitch_amp_scale=1.0):
    ...
    for i in range(NUM_SAMPLES):
        ...
        phi_ref, theta_ref, psi_ref = attitude_references(t_now, pitch_amp_scale=pitch_amp_scale)
...
def plot_dataset(dataset, dataset_label):
    ...
    axs[1, 0].plot(time, dataset["theta"], label="theta", linewidth=1, color="c")
    axs[1, 0].plot(time, dataset["theta_ref"], "--", label="theta_ref", linewidth=1, color="k")
    axs[1, 0].set_title("Pitch (state)", fontsize=11)
    axs[1, 0].set_xlabel("Time (s)")
    axs[1, 0].set_ylabel("rad")
    axs[1, 0].grid(True, linestyle="--", alpha=0.7)
    axs[1, 0].legend(fontsize=8)

    axs[1, 1].plot(time, dataset["tau_y"], label="tau_y", linewidth=1, color="tab:green")
    axs[1, 1].set_title("Pitch control tau_y (PID)", fontsize=11)
    ...
    axs[2, 0].set_title("Roll/Yaw (PID only)", fontsize=11)
...
def plot_learning_curve(history):
    ...
    plt.grid(alpha=0.3)
    plt.legend()
    plt.title("GRU learning curve (pitch axis)")
    plt.tight_layout()
    fig.savefig(os.path.join(FIG_DIR, f"{SAVE_PREFIX}step2_pitch_learning_curve.png"), dpi=300)
    plt.close(fig)


def plot_pitch_results(dataset_label, time_seq, y_true, y_pred, error_seq, split_idx):
    ...
    plt.xlabel("Time [s]")
    plt.suptitle(f"{dataset_label}: Pitch control (Train/Val/Test)")
    plt.tight_layout(rect=[0, 0.03, 1, 0.97])
    fig.savefig(os.path.join(FIG_DIR, f"{SAVE_PREFIX}{dataset_label}_step2_pitch_controls.png"), dpi=300)
    plt.close(fig)
    ...
    plt.grid(alpha=0.3)
    plt.xlabel("Time [s]")
    plt.suptitle(f"{dataset_label}: Pitch error input")
    plt.tight_layout(rect=[0, 0.03, 1, 0.97])
    fig.savefig(os.path.join(FIG_DIR, f"{SAVE_PREFIX}{dataset_label}_step2_pitch_error_inputs.png"), dpi=300)
    plt.close(fig)


def simulate_with_model(reference_z, pitch_artifacts, seq_len, Ts, wind_force=0.0, wind_start_time=20.0, wind_end_time=None, pitch_amp_scale=1.0):
    model = pitch_artifacts["model"]
    scaler_X = pitch_artifacts["scaler_X"]
    scaler_Y = pitch_artifacts["scaler_Y"]
    feature_dim = pitch_artifacts["feature_dim"]
    model_device = pitch_artifacts["device"]
    ...
    for i in range(n):
        ...
        # Pitch axis uses GRU instead of PID.
        ...
        hist_len = len(pitch_buf["err"])
        if hist_len < seq_len:
            pad_len = seq_len - hist_len
            seq_meas = np.concatenate([np.zeros(pad_len), np.array(pitch_buf["meas"])])
            seq_err = np.concatenate([np.zeros(pad_len), np.array(pitch_buf["err"])])
            seq_rate = np.concatenate([np.zeros(pad_len), np.array(pitch_buf["rate"])])
            seq_int = np.concatenate([np.zeros(pad_len), np.array(pitch_buf["int"])])
        else:
            seq_meas = np.array(pitch_buf["meas"][-seq_len:])
            seq_err = np.array(pitch_buf["err"][-seq_len:])
            seq_rate = np.array(pitch_buf["rate"][-seq_len:])
            seq_int = np.array(pitch_buf["int"][-seq_len:])
        ...
        tau_y = float(scaler_Y.inverse_transform(pred_scaled)[0, 0])
        tau_y = float(np.clip(tau_y, -PITCH_TAU_CLIP, PITCH_TAU_CLIP))
        # tau_x remains PID while tau_y comes from GRU.
...
def plot_pid_vs_model(dataset_label, time, pid_data, model_data):
    ...
    axs[0, 0].plot(time, pid_data["theta"], label="PID theta", linewidth=1, color="b")
    axs[0, 0].plot(time, model_data["theta"], label="GRU theta", linewidth=1, color="g")
    axs[0, 0].plot(time, pid_data["theta_ref"], "--", label="theta_ref", linewidth=1, color="r")
    axs[0, 0].set_title(f"{dataset_label}: Pitch tracking")
    axs[0, 0].set_xlabel("Time (s)")
    axs[0, 0].set_ylabel("theta (rad)")
    axs[0, 0].grid(True, linestyle="--", alpha=0.7)
    axs[0, 0].legend(fontsize=8)

    axs[0, 1].plot(time, pid_data["tau_y"], label="PID tau_y", linewidth=1, color="b")
    axs[0, 1].plot(time, model_data["tau_y"], label="GRU tau_y", linewidth=1, color="g")
    axs[0, 1].set_title("Pitch control")
    axs[0, 1].set_xlabel("Time (s)")
...
def main():
    print("\nStep 1: Nonlinear PID datasets (pitch-focus)")
    print(f"Using highest disturbance for C65: {MAX_WIND_FORCE} N")

    train_datasets = []
    val_datasets = []
    mat_data = {
        "run_type": np.array(["train"], dtype=object),
        "model_tag": np.array(["C65_pitch_from_C62"], dtype=object),
        ...
        "config_pitch_train_amp_scale": np.array([TRAIN_EXPERIMENT_SPECS[0]["pitch_amp_scale"]], dtype=float),
        "config_pitch_val_amp_scale": np.array([VALIDATION_EXPERIMENT_SPECS[0]["pitch_amp_scale"]], dtype=float),
    }
    ...
    for cfg in TRAIN_EXPERIMENT_SPECS:
        ...
        label = f"TRN_{build_dataset_label(wind_force, ref_seed, pitch_amp_scale)}"
        ...
        print(f"{label}: RMS pitch error = {rms_pitch:.4e} rad")
    ...
    for cfg in VALIDATION_EXPERIMENT_SPECS:
        ...
        wind_force = float(cfg["wind_force"])
        pitch_amp_scale = float(cfg["pitch_amp_scale"])
        label = f"VAL_{build_dataset_label(wind_force, ref_seed, pitch_amp_scale)}"
        ds = run_nonlinear_pid(
            wind_force=wind_force,
            wind_start_time=WIND_START_TIME,
            wind_end_time=WIND_END_TIME,
    ...
    print("\nStep 2: Train GRU on pitch PID data")
    ...
    model = PitchGRURegressor(feature_dim, HIDDEN_SIZE, 1, num_layers=NUM_LAYERS, dropout=DROPOUT).to(device)
    ...
    ckpt = {
        ...
        "feature_names": ["theta", "pitch_error", "pitch_error_rate", "pitch_error_integral"],
        "target_name": "tau_y",
    ...
    print(f"Saved pitch GRU checkpoint to: {ckpt_path}")
    ...
    for label in plot_labels:
        ...
        error_seq = ds["pitch_error"][SEQUENCE_LENGTH - 1:]
        ...
        if ENABLE_PYTHON_PLOTS:
            plot_pitch_results(label, time_seq, Y_seq, preds, error_seq, split_map[label])
    ...
    print("\nStep 3: Test pitch GRU vs fixed PID")
    model.eval()
    pitch_artifacts = {
        "model": model,
        "scaler_X": scaler_X,
