#!/usr/bin/env python3
"""
Step 1: Linearized Z-axis PID simulation (small-angle approximation).
Step 2: Train GRU to imitate the fixed PID on the Z-axis.
Generates three datasets (in memory only) and plots Z tracking + U1, then trains and plots.
"""
...
# Control noise options (same style as C42_quad_sim_data_checked.py)
...
PLOT_DATASET_LABEL = "D1"  # D1/D2/D3/ALL
...
def plot_dataset(dataset, title):
    ...
    axs[0].plot(time, dataset["z"], label="z", linewidth=2, color="b")
    axs[0].plot(time, dataset["z_ref"], "--", label="z_ref", linewidth=2, color="r")
    axs[0].set_title(title, fontsize=11, fontweight="bold")
    axs[0].set_xlabel("Time (s)", fontsize=10)
    axs[0].set_ylabel("Altitude (m)", fontsize=10)
    axs[0].grid(True, linestyle="--", alpha=0.7)
    axs[0].legend(fontsize=8)

    axs[1].plot(time, dataset["u1"], label="U1", linewidth=2, color="g")
...
def plot_pid_vs_model(dataset_label, time, z_ref, pid_data, model_data):
    ...
    axs[0].plot(time, pid_data["z"], label="PID z", linewidth=2, color="b")
    axs[0].plot(time, model_data["z"], label="Model z", linewidth=2, color="g")
    axs[0].plot(time, z_ref, "--", label="z_ref", linewidth=2, color="r")
    ...
    axs[1].plot(time, pid_data["u1"], label="PID U1", linewidth=2, color="b")
    axs[1].plot(time, model_data["u1"], label="Model U1", linewidth=2, color="g")
    axs[1].set_title("Control Input U1", fontsize=11)
    axs[1].set_xlabel("Time (s)", fontsize=10)
    axs[1].set_ylabel("U1 (N)", fontsize=10)
    axs[1].grid(True, linestyle="--", alpha=0.7)
...
def main():
    ...
    for idx in (1, 2, 3):
        ...
        plot_dataset(ds, f"Dataset {idx}: Z Tracking")
