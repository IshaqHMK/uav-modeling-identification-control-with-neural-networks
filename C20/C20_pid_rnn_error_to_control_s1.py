"""
C20 s1: Independent-axis PID imitation with explicit PID feature channels.

This script extends the s01 variant by adding integral and derivative terms to the attitude-error inputs,
so each roll/pitch/yaw RNN receives [`e`, `de/dt`, `integral e dt`] history when learning to predict the PID outputs (u2, u3, u4).
...
2. Fit feature/target standardization and train an axis-specific RNN on each PID command channel using the expanded feature stack.
3. Evaluate and store the trained models plus scalers.
"""
...
def load_and_build_dataset(mat_path: str):
    ...
    # --------------------- E1 (Step 1: add PID derivative/integral feature channels)
    ...
    # PID feature stack: current error, finite-difference derivative, and accumulated integral.
...
def main():
    ...
    # ---------- Data preparation ----------------------------------------------------
    # --------------------- E2 (Step 1: consume augmented feature stack)
    ...
    # --------------------- E3 (Step 1: sequence windows now carry expanded PID features)
    ...
    # --------------------- E4 (Step 1: keep raw error for legacy plots while models use augmented inputs)
