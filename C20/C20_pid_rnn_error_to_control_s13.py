"""
WORK IN PROGRESS! (one NN)


C20 s13: Single-head PID imitation with variable-step feature channels.

This script keeps the variable-step derivative/integral features from s12 but trains one multi-output RNN to predict
all three PID commands (u2, u3, u4) simultaneously. The data preparation, plotting, and checkpoint flow matches the
C20_pid_rnn_error_to_control_v3_clnd baseline while incorporating the richer PID-inspired inputs.
...
2. Fit feature/target standardization jointly across the three PID channels and train a single multi-output RNN.
3. Evaluate on held-out data, generate the same comparison plots, and store the trained model plus scalers on disk.
"""
...
def load_and_build_dataset(mat_path: str):
    ...
    # %----- Edit 1 (Step 3: derive PID error using raw attitude/reference)
    ...
    # %----- Edit 2 (Step 3: variable-step derivative using per-interval dt)
    error_rate = np.zeros_like(error_rad)
    error_rate[1:] = np.diff(error_rad, axis=0) / dt_samples[:, None]

    # %----- Edit 3 (Step 3: trapezoidal integral respecting unequal sampling)
...
def evaluate_sequences(model, X_np, scaler_Y, device, batch_size=2048):
    """Run batched inference over a numpy array and inverse-scale the multi-output predictions."""
    model.eval()
    preds_scaled_chunks = []
    with torch.no_grad():
...
def main():
    ...
    # ---------- Data preparation ----------------------------------------------------
    # %----- Edit 4 (Step 3: consume augmented feature stack)
    ...
    # %----- Edit 5 (Step 3: sequence windows include expanded PID features)
    ...
    # %----- Edit 6 (Step 3: keep raw error for legacy plots while models use augmented inputs)
    ...
    # %----- Edit 7 (Step 4: jointly scale multi-axis PID targets)
    ...
    # %----- Edit 8 (Step 4: single multi-output RNN replaces axis-specific models)
    model = RNNRegressor(
    ...
    # %----- Edit 9 (Step 4: single model learning curves)
    ...
    # %----- Edit 10 (Step 4: checkpoint single multi-output model)
    ckpt = {
