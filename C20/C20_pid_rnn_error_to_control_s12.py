"""
C20 s12: Independent-axis PID imitation with variable-step PID feature channels.

This script extends the s1 variant by recomputing the derivative and integral features with the actual time stamps,
so each roll/pitch/yaw RNN sees error history scaled by variable sampling intervals (finite-difference / trapezoidal updates).
ref: C20_pid_rnn_error_to_control_v3_clnd while
there are three separately trained sequence-to-command models (one per axis).
...
2. Fit feature/target standardization, compute per-axis PID targets, and train an RNN for each command channel.
3. Evaluate and store the trained models.
"""
