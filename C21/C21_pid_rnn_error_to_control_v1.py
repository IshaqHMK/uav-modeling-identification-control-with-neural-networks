"""
C21 v1: Independent-axis PID imitation with variable-step PID feature channels plus diagnostics.

This variant keeps the variable-step PID feature computation from s12 and layers in two additions:
- a diagnostic plot that overlays the error, its derivative, and its integral across the collected log
- wall-clock timing that reports how long each epoch and the full training run take per axis, plus the end-to-end runtime
...
2. Fit feature/target standardization, compute per-axis PID targets, and train an RNN for each command channel while timing each epoch.
3. Evaluate, plot the new diagnostics, record timing summaries, and store the trained models.
"""
...
SEQUENCE_LENGTH = 20            # Number of time steps used in each sliding window sequence
...
def load_and_build_dataset(mat_path: str):
    ...
    # %----- E1 (Step 3: derive PID error using raw attitude/reference)
    ...
    # %----- E2 (Step 3: variable-step derivative using per-interval dt)
    error_rate = np.zeros_like(error_rad)
    error_rate[1:] = np.diff(error_rad, axis=0) / dt_samples[:, None]

    # %----- E3 (Step 3: trapezoidal integral respecting variable sampling)
...
def main():
    ...
    # ---------- Data preparation ----------------------------------------------------
    # %----- E4 (Step 3: consume augmented feature stack)
    feature_stack, Y, time_vec, error_rad, error_rate, error_integral = load_and_build_dataset(MAT_PATH)
    print(f'Raw feature shape: {feature_stack.shape} | Raw target shape: {Y.shape}')

    # %----- E5 (Step 3: sequence windows include expanded PID features)
    ...
    # %----- E6 (Step 3: keep raw error for plots while models use augmented inputs)
    error_aligned = error_rad[SEQUENCE_LENGTH - 1:]                                 # Raw error for plotting context
    error_rate_aligned = error_rate[SEQUENCE_LENGTH - 1:]
    error_integral_aligned = error_integral[SEQUENCE_LENGTH - 1:]
    del X_seq
    ...
    for axis_name, control_short, target_name, plot_label, axis_idx in AXES:
        ...
        average_epoch_time = float(np.mean(epoch_durations)) if epoch_durations else 0.0
        print(
            f"{log_prefix}Total training time {total_elapsed:.2f}s | Avg epoch {average_epoch_time:.2f}s",
            flush=True,
        )
    ...
    total_training_seconds = sum(info["total_seconds"] for info in axis_histories.values())
    print(f"\nAggregate training time across axes: {total_training_seconds:.2f}s\n")
    ...
    fig1 = plt.figure(num="C21: Test-set Controls (True vs Pred)", figsize=PLOT_FIGSIZE)
    ...
    fig2 = plt.figure(num="C21: Full Controls (True vs Pred)", figsize=PLOT_FIGSIZE)
    for idx, (_, _, _, plot_label, _) in enumerate(AXES):
        plt.subplot(3, 1, idx + 1)
    ...
    fig3 = plt.figure(num="C21: Attitude Error Inputs", figsize=PLOT_FIGSIZE)
    ...
    # Diagnostic plot: compare the error, its derivative, and integral across all axes.
    error_rate_aligned_deg_s = np.rad2deg(error_rate_aligned)
    error_integral_aligned_deg_s = np.rad2deg(error_integral_aligned)
    fig4 = plt.figure(num="C21: Error Feature Breakdown", figsize=PLOT_FIGSIZE)
    feature_series = [
        (error_aligned_deg, "Error [deg]"),
        (error_rate_aligned_deg_s, "Error derivative [deg/s]"),
        (error_integral_aligned_deg_s, "Error integral [deg*s]"),
    ]
    for subplot_idx, (series, ylabel) in enumerate(feature_series):
        plt.subplot(3, 1, subplot_idx + 1)
        for axis_idx, (axis_name, _, _, _, _) in enumerate(AXES):
            plt.plot(time_seq, series[:, axis_idx], label=axis_name.capitalize(), linewidth=1)
        plt.ylabel(ylabel)
        plt.grid(alpha=0.3)
        if subplot_idx == 0:
            plt.legend(loc='upper right')
    plt.xlabel('Time [s]')
    plt.tight_layout()
    fig4.savefig(f"{SAVE_PREFIX}error_feature_breakdown.png", dpi=300)
    print(f"Saved {SAVE_PREFIX}error_feature_breakdown.png")

    # Overlay learning curves for each axis  
    fig5 = plt.figure(num="C21: Learning Curves", figsize=(6, 4))
    ...
    print(f"End-to-end script runtime: {script_elapsed:.2f}s")
