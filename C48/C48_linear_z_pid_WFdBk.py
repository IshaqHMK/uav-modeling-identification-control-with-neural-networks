...
Step 1:
  ...
  - Apply a fixed reference profile with three amplitude levels (<= 2.0).
...
# Figure saving
SAVE_PREFIX = "C48_WFdBk_"
FIG_DIR = os.path.dirname(os.path.abspath(__file__))
MODEL_SAVE_PREFIX = "C48_linear_z_pid_WFdBk_trainedGRUmodel"
...
def reference_profile(t, profile_id, amplitude=1.0):
    """Step 1: Reference generator for each dataset profile."""
    if profile_id == 1:
        # Multi-step: 0 -> 0.5A -> 1.0A -> 0.2A -> 0
        if t < 5.0:
            return 0.0
        if t < 15.0:
        ...
        return 0.0
    if profile_id == 2:
        # Sine (amplitude A) after 2 s
        if t < 2.0:
            return 0.0
        return amplitude * np.sin(2 * np.pi * 0.1 * (t - 2.0))
    # profile_id == 3
    # Continuous cosine between 0 and A after 2 s
    if t < 2.0:
        return 0.0
    t_rel = t - 2.0
    freq = 0.05  # Hz
    return amplitude * 0.5 * (1 - np.cos(2 * np.pi * freq * t_rel))
...
def run_linear_z_pid(profile_id, amplitude=1.0, wind_force=0.0, wind_start_time=20.0):
    ...
    for i, t in enumerate(time):
        z_ref = reference_profile(t, profile_id, amplitude=amplitude)
...
def main():
    ...
    ckpt = {
        ...
        "reference": {"profile_id": REFERENCE_PROFILE_ID, "amplitudes": REFERENCE_AMPLITUDES},
        "history": history,
    }
