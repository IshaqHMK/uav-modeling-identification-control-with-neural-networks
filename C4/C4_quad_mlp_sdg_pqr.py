# ================================================
# C4: MLP Direct Model (angles + rates)
...
def main():
    ...
    # 4.1) Simple training status diagnostic (overfit/underfit/learning)
    def diagnose_fit(train_hist, test_hist):
        th = np.array(train_hist, dtype=float)
        vh = np.array(test_hist, dtype=float)
        final_train, final_val = th[-1], vh[-1]
        min_val = float(vh.min()); best_ep = int(vh.argmin()) + 1
        gap_ratio = final_val / max(final_train, 1e-12)
        improving = False
        if len(vh) > 10:
            improving = vh[-1] < 0.98 * vh[-11]  # >2% drop over last 10 epochs

        status = "Balanced"
        # Overfitting: val has risen >5% from its minimum, while train improved after that point
        if final_val > 1.05 * min_val and final_train < th[best_ep-1] * 0.98:
            status = "Overfitting likely"
        # Still improving if both curves trending down recently
        elif improving and th[-1] < 0.98 * th[-11]:
            status = "Still improving"
        # Underfitting if both losses are high and close together (no gap and not decreasing much)
        elif gap_ratio < 1.1 and (len(vh) < 11 or vh[-1] > 0.95 * vh[max(len(vh)-11,0)]):
            status = "Possible underfitting"

        print("\n=== Training Diagnostic ===")
        print(f"Best val @ epoch {best_ep}: {min_val:.4e}")
        print(f"Final train: {final_train:.4e}  Final val: {final_val:.4e}  Gap ratio: {gap_ratio:.2f}")
        print(f"Status: {status}")
        if status == "Overfitting likely":
            print("Tips: lower hid_dim or depth, add dropout/weight_decay, or early stop at best epoch.")
        elif status == "Possible underfitting":
            print("Tips: increase hid_dim, train longer, or try a higher LR briefly.")
        elif status == "Still improving":
            print("Tips: keep training or enable early stopping with patience.")
        else:
            print("Looks reasonable. Consider early stopping around best epoch.")

    diagnose_fit(train_hist, test_hist)
    ...
    rmse_angles = np.sqrt(mean_squared_error(A_true, A_pred, multioutput='raw_values'))
    rmse_rates  = np.sqrt(mean_squared_error(R_true, R_pred, multioutput='raw_values'))
    print(f"RMSE angles [deg]:  phi={rmse_angles[0]:.3f}  theta={rmse_angles[1]:.3f}  psi={rmse_angles[2]:.3f}")
    print(f"RMSE rates  [deg/s]: p={rmse_rates[0]:.3f}  q={rmse_rates[1]:.3f}  r={rmse_rates[2]:.3f}")
    ...
    fig1 = plt.figure(num="C4: Test-set Angles (True vs Pred)", figsize=PLOT_FIGSIZE)
    ...
    # Rates — test set
    fig2 = plt.figure(num="C4: Test-set Rates (True vs Pred)", figsize=PLOT_FIGSIZE)
    ...
    fig3 = plt.figure(num="C4: Full Angles (True vs Pred)", figsize=PLOT_FIGSIZE)
    ...
    # Rates — full sequence
    fig4 = plt.figure(num="C4: Full Rates (True vs Pred)", figsize=PLOT_FIGSIZE)
    ...
    # Learning curves
    fig5 = plt.figure(num="C4: Learning Curves", figsize=(6,4))
    ...
    ckpt = {
        "model_class": "MLP",
        "model_kwargs": {"inp_dim": 9, "hid_dim": 16, "out_dim": 6, "depth": 2, "dropout": 0.2},
